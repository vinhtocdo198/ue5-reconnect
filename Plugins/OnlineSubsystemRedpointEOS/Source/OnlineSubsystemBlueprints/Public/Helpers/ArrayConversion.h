...
/**
 * Converts a set of one type into a set of another type. This helper function is used in auto-generated
 * code when we need to convert sets.
 */
template <typename TSrc, typename TDest>
TSet<TDest> ConvertSetElements(const TSet<TSrc> &Src, std::function<TDest(const TSrc &Src)> Convert)
{
    TSet<TDest> Dest;
...
/**
 * Validates that all the elements of a set are valid.
 */
template <typename TSrc> bool ValidateSetElements(const TSet<TSrc> &Src, std::function<bool(const TSrc &Src)> Validate)
{
    for (const auto &Elem : Src)
    {
        if (!Validate(Elem))
