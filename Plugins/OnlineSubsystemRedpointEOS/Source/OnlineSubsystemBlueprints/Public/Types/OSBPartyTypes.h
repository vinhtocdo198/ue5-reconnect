...
struct ONLINESUBSYSTEMBLUEPRINTS_API FOnlinePartyConfiguration
{
    GENERATED_BODY()

public:
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    EJoinRequestAction_ JoinRequestAction = EJoinRequestAction_::AutoApprove;

    /* Who can see this party and join it through presence? */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    EPartySystemPermissions PresencePermissions = EPartySystemPermissions::Anyone;

    /* Who can send out invites to other users when they're in this party? */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    EPartySystemPermissions InvitePermissions = EPartySystemPermissions::Anyone;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    bool ChatEnabled = false;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    bool ShouldRemoveOnDisconnection = false;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    bool IsAcceptingMembers = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Online")
    int32 NotAcceptingMembersReason = 0;
...
public:
    ...
    UPartyId *GetPartyId() const;

    UFUNCTION(BlueprintCallable, Category = "Online")
    int64 GetPartyTypeId() const;

    UFUNCTION(BlueprintCallable, Category = "Online")
    FUniqueNetIdRepl GetLeaderId() const;

    UFUNCTION(BlueprintCallable, Category = "Online")
    FOnlinePartyConfiguration GetPartyConfiguration() const;

    static UParty *FromNative(const FOnlinePartyConstPtr &InObj);
...
public:
    ...
    static UBlueprintPartyMember *FromNative(const FOnlinePartyMemberConstPtr &InObj);
...
public:
    ...
    static UReadablePartyData *FromNative(const TSharedPtr<const FOnlinePartyData> &InObj);
...
public:
    ...
    static UMutablePartyData *FromNative(const TSharedRef<FOnlinePartyData> &InObj);
    static UMutablePartyData *FromNative(const TSharedPtr<FOnlinePartyData> &InObj);
    ...
    UFUNCTION(BlueprintCallable, Category = "Online")
    void SetAttribute(const FString &AttrName, const FVariantDataBP &AttrValue);
