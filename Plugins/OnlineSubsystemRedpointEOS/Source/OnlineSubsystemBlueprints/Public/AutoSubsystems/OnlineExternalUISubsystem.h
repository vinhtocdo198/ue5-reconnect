...
// DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FExternalUI_LoginFlowUIRequired_BP, const FString&, RequestedURL, const
// FOnLoginRedirectURL&, OnRedirectURL, const FOnLoginFlowComplete&, OnLoginFlowComplete, bool&,
// bOutShouldContinueLogin);

// @parameters-not-compatible
// DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FExternalUI_CreateAccountFlowUIRequired_BP, const FString&,
// RequestedURL, const FOnLoginRedirectURL&, OnRedirectURL, const FOnLoginFlowComplete&, OnLoginFlowComplete, bool&,
// bOutShouldContinueLogin);
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Online|ExternalUI")
    void LoginFlowComplete(int RequestID, const FLoginFlowResultBP &Result);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
    FOnlineExternalUISubsystemShowLoginUICallbackPin,
...
private:
    UPROPERTY()
    TObjectPtr<UOnlineExternalUISubsystem> __Store__Subsystem;
    UPROPERTY()
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
    FOnlineExternalUISubsystemShowAccountCreationUICallbackPin,
    int,
    ControllerIndex,
    const FOnlineAccountCredential &,
    AccountCredentials,
    const FOnlineErrorInfo &,
    Error);
...
private:
    ...
    UPROPERTY()
    FString __Store__CallUniquenessId;
    void HandleCallback_OnAccountCreationUIClosed(
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnlineExternalUISubsystemShowWebURLCallbackPin, const FString &, FinalUrl);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(
    FOnlineExternalUISubsystemShowSendMessageToUserUICallbackPin,
    bool,
    bMessageSent);

UCLASS()
class ONLINESUBSYSTEMBLUEPRINTS_API UOnlineExternalUISubsystemShowSendMessageToUserUI : public UBlueprintAsyncActionBase
{
    GENERATED_BODY()
