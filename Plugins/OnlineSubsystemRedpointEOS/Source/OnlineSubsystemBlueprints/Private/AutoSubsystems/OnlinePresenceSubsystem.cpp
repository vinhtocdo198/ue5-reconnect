...
FName UOnlinePresenceSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlinePresenceSubsystem::UOnlinePresenceSubsystem()
{
    ...
    this->OSSName = UOnlinePresenceSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlinePresenceSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnPresenceReceived =
            Handle->AddOnPresenceReceivedDelegate_Handle(FOnPresenceReceived::FDelegate::CreateWeakLambda(
                this,
                [this](const class FUniqueNetId &UserId, const TSharedRef<FOnlineUserPresence> &Presence) {
                    this->OnPresenceReceived.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        FOnlineUserPresenceData::FromNative(Presence));
                }));
        this->DelegateHandle_OnPresenceArrayUpdated =
            Handle->AddOnPresenceArrayUpdatedDelegate_Handle(FOnPresenceArrayUpdated::FDelegate::CreateWeakLambda(
                this,
                [this](
                    const class FUniqueNetId &UserId,
                    const TArray<TSharedRef<FOnlineUserPresence>> &NewPresenceArray) {
                    this->OnPresenceArrayUpdated.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertArrayElements<TSharedRef<FOnlineUserPresence>, FOnlineUserPresenceData>(
                            NewPresenceArray,
                            [](const TSharedRef<FOnlineUserPresence> &Val) {
                                return FOnlineUserPresenceData::FromNative(Val);
                            }));
                }));
    }
}
...
UOnlinePresenceSubsystemSetPresence *UOnlinePresenceSubsystemSetPresence::SetPresence(
    ...
    const FOnlineUserPresenceStatusData &Status)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlinePresenceSubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlinePresenceSubsystemSetPresence::Activate()
{
    ...
    if (!(this->__Store__User.IsValid() && this->__Store__User.GetUniqueNetId().IsValid()))
    {
    ...
    auto __DelegateHandle_OnPresenceTaskComplete = IOnlinePresence::FOnPresenceTaskCompleteDelegate::CreateUObject(
        this,
        &UOnlinePresenceSubsystemSetPresence::HandleCallback_OnPresenceTaskComplete);

    Handle->SetPresence(
        this->__Store__User.GetUniqueNetId().ToSharedRef().Get(),
        this->__Store__Status.ToNative(),
        __DelegateHandle_OnPresenceTaskComplete);
}

void UOnlinePresenceSubsystemSetPresence::HandleCallback_OnPresenceTaskComplete(
    const class FUniqueNetId &UserId,
...
UOnlinePresenceSubsystemQueryPresence *UOnlinePresenceSubsystemQueryPresence::QueryPresence(
    UOnlinePresenceSubsystem *Subsystem,
    const FUniqueNetIdRepl &User)
{
    if (!IsValid(Subsystem))
...
void UOnlinePresenceSubsystemQueryPresence::Activate()
{
    ...
    auto __DelegateHandle_OnPresenceTaskComplete = IOnlinePresence::FOnPresenceTaskCompleteDelegate::CreateUObject(
        this,
        &UOnlinePresenceSubsystemQueryPresence::HandleCallback_OnPresenceTaskComplete);

    Handle->QueryPresence(
        this->__Store__User.GetUniqueNetId().ToSharedRef().Get(),
        __DelegateHandle_OnPresenceTaskComplete);
}

void UOnlinePresenceSubsystemQueryPresence::HandleCallback_OnPresenceTaskComplete(
    const class FUniqueNetId &UserId,
    const bool bWasSuccessful)
{
    if (!IsValid(this->__Store__Subsystem))
...
EOnlineCachedResult_ UOnlinePresenceSubsystem::GetCachedPresence(
    const FUniqueNetIdRepl &User,
    FOnlineUserPresenceData &OutPresence)
{
    ...
    if (!(User.IsValid() && User.GetUniqueNetId().IsValid()))
    {
        return EOnlineCachedResult_::NotFound;
    }
    ...
    auto __Result = (EOnlineCachedResult_)(Handle->GetCachedPresence(
        User.GetUniqueNetId().ToSharedRef().Get(),
        __StoreTemp__OutPresence));
    OutPresence = FOnlineUserPresenceData::FromNative(__StoreTemp__OutPresence);

    return __Result;
}

EOnlineCachedResult_ UOnlinePresenceSubsystem::GetCachedPresenceForApp(
    ...
    FOnlineUserPresenceData &OutPresence)
{
    ...
    auto __Result = (EOnlineCachedResult_)(Handle->GetCachedPresenceForApp(
