...
FName UOnlineUserCloudSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineUserCloudSubsystem::UOnlineUserCloudSubsystem()
{
    ...
    this->OSSName = UOnlineUserCloudSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineUserCloudSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnEnumerateUserFilesComplete = Handle->AddOnEnumerateUserFilesCompleteDelegate_Handle(
            FOnEnumerateUserFilesComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FUniqueNetId &UserId) {
                    this->OnEnumerateUserFilesComplete.Broadcast(
                        bWasSuccessful,
                        ConvertDangerousUniqueNetIdToRepl(UserId));
                }));
        this->DelegateHandle_OnReadUserFileComplete =
            Handle->AddOnReadUserFileCompleteDelegate_Handle(FOnReadUserFileComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FUniqueNetId &UserId, const FString &FileName) {
                    this->OnReadUserFileComplete.Broadcast(
                        bWasSuccessful,
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        FileName);
                }));
#if REDPOINT_EOS_UE_5_4_OR_LATER
        this->DelegateHandle_OnWriteUserFileProgress64 =
            Handle->AddOnWriteUserFileProgress64Delegate_Handle(FOnWriteUserFileProgress64::FDelegate::CreateWeakLambda(
                this,
                [this](uint64 BytesWritten, const FUniqueNetId &UserId, const FString &FileName) {
                    this->OnWriteUserFileProgress64.Broadcast(
                        (int64)(BytesWritten),
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        FileName);
                }));
#else
        this->DelegateHandle_OnWriteUserFileProgress64 =
            Handle->AddOnWriteUserFileProgressDelegate_Handle(FOnWriteUserFileProgress::FDelegate::CreateWeakLambda(
                this,
...
#endif
        this->DelegateHandle_OnWriteUserFileComplete =
            Handle->AddOnWriteUserFileCompleteDelegate_Handle(FOnWriteUserFileComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FUniqueNetId &UserId, const FString &FileName) {
                    this->OnWriteUserFileComplete.Broadcast(
                        bWasSuccessful,
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        FileName);
                }));
        this->DelegateHandle_OnWriteUserFileCanceled =
            Handle->AddOnWriteUserFileCanceledDelegate_Handle(FOnWriteUserFileCanceled::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FUniqueNetId &UserId, const FString &FileName) {
                    this->OnWriteUserFileCanceled.Broadcast(
                        bWasSuccessful,
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        FileName);
                }));
        this->DelegateHandle_OnDeleteUserFileComplete =
            Handle->AddOnDeleteUserFileCompleteDelegate_Handle(FOnDeleteUserFileComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FUniqueNetId &UserId, const FString &FileName) {
                    this->OnDeleteUserFileComplete.Broadcast(
                        bWasSuccessful,
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        FileName);
                }));
...
#if REDPOINT_EOS_UE_5_4_OR_LATER
        Handle->ClearOnWriteUserFileProgress64Delegate_Handle(this->DelegateHandle_OnWriteUserFileProgress64);
#else
        Handle->ClearOnWriteUserFileProgressDelegate_Handle(this->DelegateHandle_OnWriteUserFileProgress64);
#endif
...
bool UOnlineUserCloudSubsystem::GetFileContents(
    ...
    UFileData *&FileContents)
{
    ...
    auto __Result =
        Handle->GetFileContents(UserId.GetUniqueNetId().ToSharedRef().Get(), FileName, __StoreTemp__FileContents);
    FileContents = UFileData::FromNative(__StoreTemp__FileContents);

    return __Result;
}

bool UOnlineUserCloudSubsystem::ClearFiles(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineUserCloudSubsystem::ClearFile(const FUniqueNetIdRepl &UserId, const FString &FileName)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
UOnlineUserCloudSubsystemEnumerateUserFiles *UOnlineUserCloudSubsystemEnumerateUserFiles::EnumerateUserFiles(
    ...
    const FUniqueNetIdRepl &UserId)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineUserCloudSubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineUserCloudSubsystemEnumerateUserFiles::Activate()
{
    ...
    this->DelegateHandle_OnEnumerateUserFilesComplete =
        Handle->AddOnEnumerateUserFilesCompleteDelegate_Handle(FOnEnumerateUserFilesCompleteDelegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemEnumerateUserFiles::HandleCallback_OnEnumerateUserFilesComplete));

    Handle->EnumerateUserFiles(this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get());
}

void UOnlineUserCloudSubsystemEnumerateUserFiles::HandleCallback_OnEnumerateUserFilesComplete(
    bool bWasSuccessful,
...
void UOnlineUserCloudSubsystem::GetUserFileList(const FUniqueNetIdRepl &UserId, TArray<FCloudFileHeaderBP> &UserFiles)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    UserFiles = ConvertArrayElements<FCloudFileHeader, FCloudFileHeaderBP>(
        __StoreTemp__UserFiles,
        [](const FCloudFileHeader &Val) {
            return FCloudFileHeaderBP::FromNative(Val);
        });
}

UOnlineUserCloudSubsystemReadUserFile *UOnlineUserCloudSubsystemReadUserFile::ReadUserFile(
    UOnlineUserCloudSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const FString &FileName)
{
    ...
    Node->__Store__Subsystem = Subsystem;
    Node->__Store__CallUniquenessId =
        FString::Printf(TEXT("OnReadUserFileComplete|%s|%s"), *UserId.ToString(), *FileName);
    Node->__Store__UserId = UserId;
...
void UOnlineUserCloudSubsystemReadUserFile::Activate()
{
    ...
    this->DelegateHandle_OnReadUserFileComplete =
        Handle->AddOnReadUserFileCompleteDelegate_Handle(FOnReadUserFileCompleteDelegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemReadUserFile::HandleCallback_OnReadUserFileComplete));
...
void UOnlineUserCloudSubsystemReadUserFile::HandleCallback_OnReadUserFileComplete(
    bool bWasSuccessful,
    const FUniqueNetId &UserId,
    const FString &FileName)
{
...
UOnlineUserCloudSubsystemWriteUserFile *UOnlineUserCloudSubsystemWriteUserFile::WriteUserFile(
    UOnlineUserCloudSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const FString &FileName,
    UFileData *FileContents,
    bool bCompressBeforeUpload)
{
    ...
    Node->__Store__Subsystem = Subsystem;
    Node->__Store__CallUniquenessId =
        FString::Printf(TEXT("OnWriteUserFileComplete|%s|%s"), *UserId.ToString(), *FileName);
    Node->__Store__UserId = UserId;
    Node->__Store__FileName = FileName;
...
void UOnlineUserCloudSubsystemWriteUserFile::Activate()
{
    ...
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem value not valid when Activate() was called"));
        this->OnCallFailed.Broadcast(false, FUniqueNetIdRepl(), TEXT(""), int64());
        return;
    }
    if (!(this->__Store__UserId.IsValid() && this->__Store__UserId.GetUniqueNetId().IsValid()))
    {
    ...
    if (!(IsValid(this->__Store__FileContents)))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Parameter 'FileContents' was not valid"));
        this->OnCallFailed.Broadcast(false, FUniqueNetIdRepl(), TEXT(""), int64());
        return;
    }
    ...
    this->DelegateHandle_OnWriteUserFileComplete =
        Handle->AddOnWriteUserFileCompleteDelegate_Handle(FOnWriteUserFileCompleteDelegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemWriteUserFile::HandleCallback_OnWriteUserFileComplete));
#if REDPOINT_EOS_UE_5_4_OR_LATER
    this->ProgressDelegateHandle_OnWriteUserFileComplete =
        Handle->AddOnWriteUserFileProgress64Delegate_Handle(FOnWriteUserFileProgress64Delegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemWriteUserFile::HandleCallbackProgress_OnWriteUserFileComplete));
#else
    this->ProgressDelegateHandle_OnWriteUserFileComplete =
        Handle->AddOnWriteUserFileProgressDelegate_Handle(FOnWriteUserFileProgressDelegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemWriteUserFile::HandleCallbackProgress_OnWriteUserFileComplete));
#endif
    this->CancelDelegateHandle_OnWriteUserFileComplete =
        Handle->AddOnWriteUserFileCanceledDelegate_Handle(FOnWriteUserFileCanceledDelegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemWriteUserFile::HandleCallbackCancel_OnWriteUserFileComplete));
    TArray<uint8> __StoreArg_FileContents =
        (IsValid(this->__Store__FileContents) ? this->__Store__FileContents->ToNative() : TArray<uint8>());

    if (!Handle->WriteUserFile(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__FileName,
            __StoreArg_FileContents,
            this->__Store__bCompressBeforeUpload))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Call 'WriteUserFile' did not start successfully"));
        Handle->ClearOnWriteUserFileCompleteDelegate_Handle(this->DelegateHandle_OnWriteUserFileComplete);
#if REDPOINT_EOS_UE_5_4_OR_LATER
        Handle->ClearOnWriteUserFileProgress64Delegate_Handle(this->ProgressDelegateHandle_OnWriteUserFileComplete);
#else
...
void UOnlineUserCloudSubsystemWriteUserFile::HandleCallback_OnWriteUserFileComplete(
    bool bWasSuccessful,
    const FUniqueNetId &UserId,
    const FString &FileName)
{
    ...
    this->OnWriteUserFileComplete
        .Broadcast(bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(UserId), FileName, int64());
}

#if REDPOINT_EOS_UE_5_4_OR_LATER
void UOnlineUserCloudSubsystemWriteUserFile::HandleCallbackProgress_OnWriteUserFileComplete(
    uint64 BytesWritten,
    const FUniqueNetId &UserId,
    const FString &FileName)
#else
...
#endif
{
    ...
    this->OnWriteUserFileProgress64
        .Broadcast(false, ConvertDangerousUniqueNetIdToRepl(UserId), FileName, (int64)(BytesWritten));
}
void UOnlineUserCloudSubsystemWriteUserFile::HandleCallbackCancel_OnWriteUserFileComplete(
    bool bWasSuccessful,
    const FUniqueNetId &UserId,
    const FString &FileName)
{
    ...
    this->OnWriteUserFileCanceled
        .Broadcast(bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(UserId), FileName, int64());
}

void UOnlineUserCloudSubsystem::CancelWriteUserFile(const FUniqueNetIdRepl &UserId, const FString &FileName)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
UOnlineUserCloudSubsystemDeleteUserFile *UOnlineUserCloudSubsystemDeleteUserFile::DeleteUserFile(
    UOnlineUserCloudSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const FString &FileName,
    ...
    bool bShouldLocallyDelete)
{
    ...
    Node->__Store__Subsystem = Subsystem;
    Node->__Store__CallUniquenessId =
        FString::Printf(TEXT("OnDeleteUserFileComplete|%s|%s"), *UserId.ToString(), *FileName);
    Node->__Store__UserId = UserId;
    Node->__Store__FileName = FileName;
...
void UOnlineUserCloudSubsystemDeleteUserFile::Activate()
{
    ...
    this->DelegateHandle_OnDeleteUserFileComplete =
        Handle->AddOnDeleteUserFileCompleteDelegate_Handle(FOnDeleteUserFileCompleteDelegate::CreateUObject(
            this,
            &UOnlineUserCloudSubsystemDeleteUserFile::HandleCallback_OnDeleteUserFileComplete));

    if (!Handle->DeleteUserFile(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__FileName,
            this->__Store__bShouldCloudDelete,
            this->__Store__bShouldLocallyDelete))
    {
...
void UOnlineUserCloudSubsystemDeleteUserFile::HandleCallback_OnDeleteUserFileComplete(
    bool bWasSuccessful,
    const FUniqueNetId &UserId,
    const FString &FileName)
{
...
bool UOnlineUserCloudSubsystem::RequestUsageInfo(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
void UOnlineUserCloudSubsystem::DumpCloudState(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
void UOnlineUserCloudSubsystem::DumpCloudFileState(const FUniqueNetIdRepl &UserId, const FString &FileName)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
