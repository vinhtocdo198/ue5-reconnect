...
UOnlineStoreV2Subsystem::UOnlineStoreV2Subsystem()
{
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineStoreV2Subsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnQueryForAvailablePurchasesComplete =
            Handle->AddOnQueryForAvailablePurchasesCompleteDelegate_Handle(
...
void UOnlineStoreV2Subsystem::BeginDestroy()
{
    ...
    if (Handle.IsValid())
    {
        ...
        Handle->ClearOnQueryForAvailablePurchasesCompleteDelegate_Handle(
            this->DelegateHandle_OnQueryForAvailablePurchasesComplete);
    }
...
UOnlineStoreV2SubsystemQueryCategories *UOnlineStoreV2SubsystemQueryCategories::QueryCategories(
    ...
    const FUniqueNetIdRepl &UserId)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineStoreV2Subsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineStoreV2SubsystemQueryCategories::Activate()
{
    ...
    auto __DelegateHandle_OnQueryOnlineStoreCategoriesComplete = FOnQueryOnlineStoreCategoriesComplete::CreateUObject(
        this,
        &UOnlineStoreV2SubsystemQueryCategories::HandleCallback_OnQueryOnlineStoreCategoriesComplete);

    Handle->QueryCategories(
        this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
        __DelegateHandle_OnQueryOnlineStoreCategoriesComplete);
}

void UOnlineStoreV2SubsystemQueryCategories::HandleCallback_OnQueryOnlineStoreCategoriesComplete(
    bool bWasSuccessful,
    const FString &Error)
{
...
void UOnlineStoreV2Subsystem::GetCategories(TArray<FOnlineStoreCategoryBP> &OutCategories)
{
    ...
    OutCategories = ConvertArrayElements<FOnlineStoreCategory, FOnlineStoreCategoryBP>(
        __StoreTemp__OutCategories,
        [](const FOnlineStoreCategory &Val) {
            return FOnlineStoreCategoryBP::FromNative(Val);
        });
}

UOnlineStoreV2SubsystemQueryOffersByFilter *UOnlineStoreV2SubsystemQueryOffersByFilter::QueryOffersByFilter(
    UOnlineStoreV2Subsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const FOnlineStoreFilterBP &Filter)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
...
void UOnlineStoreV2SubsystemQueryOffersByFilter::Activate()
{
    ...
    auto __DelegateHandle_OnQueryOnlineStoreOffersComplete = FOnQueryOnlineStoreOffersComplete::CreateUObject(
        this,
        &UOnlineStoreV2SubsystemQueryOffersByFilter::HandleCallback_OnQueryOnlineStoreOffersComplete);

    Handle->QueryOffersByFilter(
        this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
        this->__Store__Filter.ToNative(),
        __DelegateHandle_OnQueryOnlineStoreOffersComplete);
}

void UOnlineStoreV2SubsystemQueryOffersByFilter::HandleCallback_OnQueryOnlineStoreOffersComplete(
    bool bWasSuccessful,
    ...
    const FString &Error)
{
    ...
    this->OnQueryOnlineStoreOffersComplete.Broadcast(
        bWasSuccessful,
        ConvertArrayElements<FUniqueOfferId, FString>(
            OfferIds,
            [](const FUniqueOfferId &Val) {
                return Val;
            }),
        Error);
}

UOnlineStoreV2SubsystemQueryOffersById *UOnlineStoreV2SubsystemQueryOffersById::QueryOffersById(
    UOnlineStoreV2Subsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
...
void UOnlineStoreV2SubsystemQueryOffersById::Activate()
{
    ...
    if (!(ValidateArrayElements<FString>(this->__Store__OfferIds, [](const FString &Val) {
            return true;
        })))
    {
    ...
    auto __DelegateHandle_OnQueryOnlineStoreOffersComplete = FOnQueryOnlineStoreOffersComplete::CreateUObject(
        this,
        &UOnlineStoreV2SubsystemQueryOffersById::HandleCallback_OnQueryOnlineStoreOffersComplete);

    Handle->QueryOffersById(
        this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
        ConvertArrayElements<FString, FUniqueOfferId>(
            this->__Store__OfferIds,
            [](const FString &Val) {
                return Val;
            }),
        __DelegateHandle_OnQueryOnlineStoreOffersComplete);
...
void UOnlineStoreV2SubsystemQueryOffersById::HandleCallback_OnQueryOnlineStoreOffersComplete(
    bool bWasSuccessful,
    const TArray<FUniqueOfferId> &OfferIds,
    const FString &Error)
{
...
void UOnlineStoreV2Subsystem::GetOffers(TArray<UOnlineStoreOffer *> &OutOffers)
{
    ...
    OutOffers = ConvertArrayElements<FOnlineStoreOfferRef, UOnlineStoreOffer *>(
        __StoreTemp__OutOffers,
        [](const FOnlineStoreOfferRef &Val) {
            return UOnlineStoreOffer::FromNative(Val);
        });
}

UOnlineStoreOffer *UOnlineStoreV2Subsystem::GetOffer(const FString &OfferId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
