...
FName UOnlineStatsSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineStatsSubsystem::UOnlineStatsSubsystem()
{
    ...
    this->OSSName = UOnlineStatsSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineStatsSubsystem subsystem initialized in context %s"),
...
UOnlineStatsSubsystemQueryStats *UOnlineStatsSubsystemQueryStats::QueryStats(
    UOnlineStatsSubsystem *Subsystem,
    const FUniqueNetIdRepl &LocalUserId,
    const TArray<FUniqueNetIdRepl> &StatUsers,
    const TArray<FString> &StatNames)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineStatsSubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineStatsSubsystemQueryStats::Activate()
{
    ...
    if (!(ValidateArrayElements<FUniqueNetIdRepl>(this->__Store__StatUsers, [](const FUniqueNetIdRepl &Val) {
    ...
    if (!(ValidateArrayElements<FString>(this->__Store__StatNames, [](const FString &Val) {
            return true;
        })))
    {
    ...
    auto __DelegateHandle_OnlineStatsQueryUsersStatsComplete = FOnlineStatsQueryUsersStatsComplete::CreateUObject(
        this,
        &UOnlineStatsSubsystemQueryStats::HandleCallback_OnlineStatsQueryUsersStatsComplete);

    Handle->QueryStats(
        this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef(),
        ConvertArrayElements<FUniqueNetIdRepl, FUniqueNetIdRef>(
            this->__Store__StatUsers,
            [](const FUniqueNetIdRepl &Val) {
                return Val.GetUniqueNetId().ToSharedRef();
            }),
        ConvertArrayElements<FString, FString>(
            this->__Store__StatNames,
            [](const FString &Val) {
                return Val;
            }),
        __DelegateHandle_OnlineStatsQueryUsersStatsComplete);
}

void UOnlineStatsSubsystemQueryStats::HandleCallback_OnlineStatsQueryUsersStatsComplete(
    ...
    const TArray<TSharedRef<const FOnlineStatsUserStats>> &UsersStatsResult)
{
    ...
    this->OnlineStatsQueryUsersStatsComplete.Broadcast(
        FOnlineErrorInfo::FromNative(ResultState),
        ConvertArrayElements<TSharedRef<const FOnlineStatsUserStats>, FOnlineStatsUserStatsBP>(
            UsersStatsResult,
            [](const TSharedRef<const FOnlineStatsUserStats> &Val) {
                return FOnlineStatsUserStatsBP::FromNative(Val);
            }));
}

FOnlineStatsUserStatsBP UOnlineStatsSubsystem::GetStats(const FUniqueNetIdRepl &StatsUserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    ...
    if (!(StatsUserId.IsValid() && StatsUserId.GetUniqueNetId().IsValid()))
    {
        return FOnlineStatsUserStatsBP();
    }
...
UOnlineStatsSubsystemUpdateStats *UOnlineStatsSubsystemUpdateStats::UpdateStats(
    UOnlineStatsSubsystem *Subsystem,
    const FUniqueNetIdRepl &LocalUserId,
    const TArray<FOnlineStatsUserUpdatedStatsBP> &UpdatedUserStats)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
...
void UOnlineStatsSubsystemUpdateStats::Activate()
{
    ...
    if (!(ValidateArrayElements<FOnlineStatsUserUpdatedStatsBP>(
            this->__Store__UpdatedUserStats,
            [](const FOnlineStatsUserUpdatedStatsBP &Val) {
                return true;
            })))
    {
    ...
    auto __DelegateHandle_OnlineStatsUpdateStatsComplete = FOnlineStatsUpdateStatsComplete::CreateUObject(
        this,
        &UOnlineStatsSubsystemUpdateStats::HandleCallback_OnlineStatsUpdateStatsComplete);

    Handle->UpdateStats(
        this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef(),
        ConvertArrayElements<FOnlineStatsUserUpdatedStatsBP, FOnlineStatsUserUpdatedStats>(
            this->__Store__UpdatedUserStats,
            [](const FOnlineStatsUserUpdatedStatsBP &Val) {
                return Val.ToNative();
            }),
        __DelegateHandle_OnlineStatsUpdateStatsComplete);
}
