...
FName UOnlineFriendsSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineFriendsSubsystem::UOnlineFriendsSubsystem()
{
    ...
    this->OSSName = UOnlineFriendsSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineFriendsSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
            ...
            this->DelegateHandle_OnRejectInviteComplete.Add(Handle->AddOnRejectInviteCompleteDelegate_Handle(
                i,
                FOnRejectInviteComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this,
                     i](int32 _UnusedLocalUserNum,
                        ...
                        this->OnRejectInviteComplete.Broadcast(
                            i,
                            bWasSuccessful,
                            ConvertDangerousUniqueNetIdToRepl(FriendId),
                            ListName,
                            ErrorStr);
                    })));
            this->DelegateHandle_OnDeleteFriendComplete.Add(Handle->AddOnDeleteFriendCompleteDelegate_Handle(
                i,
                FOnDeleteFriendComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this,
                     i](int32 _UnusedLocalUserNum,
                        bool bWasSuccessful,
                        ...
                        const FString &ListName,
                        const FString &ErrorStr) {
                        this->OnDeleteFriendComplete.Broadcast(
                            i,
                            bWasSuccessful,
                            ConvertDangerousUniqueNetIdToRepl(FriendId),
                            ListName,
            ...
            this->DelegateHandle_OnBlockedPlayerComplete.Add(Handle->AddOnBlockedPlayerCompleteDelegate_Handle(
                i,
                FOnBlockedPlayerComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this,
                     i](int32 _UnusedLocalUserNum,
                        bool bWasSuccessful,
                        ...
                        const FString &ListName,
                        const FString &ErrorStr) {
                        this->OnBlockedPlayerComplete.Broadcast(
                            i,
                            bWasSuccessful,
                            ConvertDangerousUniqueNetIdToRepl(UniqueId),
                            ListName,
                            ErrorStr);
                    })));
            this->DelegateHandle_OnUnblockedPlayerComplete.Add(Handle->AddOnUnblockedPlayerCompleteDelegate_Handle(
                i,
                FOnUnblockedPlayerComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this,
                     i](int32 _UnusedLocalUserNum,
                        bool bWasSuccessful,
                        ...
                        const FString &ListName,
                        const FString &ErrorStr) {
                        this->OnUnblockedPlayerComplete.Broadcast(
                            i,
                            bWasSuccessful,
                            ConvertDangerousUniqueNetIdToRepl(UniqueId),
                            ListName,
            ...
            this->DelegateHandle_OnBlockListChange.Add(Handle->AddOnBlockListChangeDelegate_Handle(
                i,
                FOnBlockListChange::FDelegate::CreateWeakLambda(
                    this,
                    [this, i](int32 _UnusedLocalUserNum, const FString &ListName) {
                        this->OnBlockListChange.Broadcast(i, ListName);
                    })));
        }
        this->DelegateHandle_OnInviteReceived =
            Handle->AddOnInviteReceivedDelegate_Handle(FOnInviteReceived::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FUniqueNetId &FriendId) {
                    this->OnInviteReceived.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertDangerousUniqueNetIdToRepl(FriendId));
                }));
        this->DelegateHandle_OnInviteAccepted =
            Handle->AddOnInviteAcceptedDelegate_Handle(FOnInviteAccepted::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FUniqueNetId &FriendId) {
                    this->OnInviteAccepted.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertDangerousUniqueNetIdToRepl(FriendId));
                }));
        this->DelegateHandle_OnInviteRejected =
            Handle->AddOnInviteRejectedDelegate_Handle(FOnInviteRejected::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FUniqueNetId &FriendId) {
                    this->OnInviteRejected.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertDangerousUniqueNetIdToRepl(FriendId));
                }));
        this->DelegateHandle_OnInviteAborted =
            Handle->AddOnInviteAbortedDelegate_Handle(FOnInviteAborted::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FUniqueNetId &FriendId) {
                    this->OnInviteAborted.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertDangerousUniqueNetIdToRepl(FriendId));
                }));
        this->DelegateHandle_OnFriendRemoved =
            Handle->AddOnFriendRemovedDelegate_Handle(FOnFriendRemoved::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FUniqueNetId &FriendId) {
                    this->OnFriendRemoved.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertDangerousUniqueNetIdToRepl(FriendId));
                }));
        this->DelegateHandle_OnQueryRecentPlayersComplete = Handle->AddOnQueryRecentPlayersCompleteDelegate_Handle(
            FOnQueryRecentPlayersComplete::FDelegate::CreateWeakLambda(
                this,
                [this](
                    ...
                    bool bWasSuccessful,
                    const FString &Error) {
                    this->OnQueryRecentPlayersComplete
                        .Broadcast(ConvertDangerousUniqueNetIdToRepl(UserId), Namespace, bWasSuccessful, Error);
                }));
        this->DelegateHandle_OnQueryBlockedPlayersComplete = Handle->AddOnQueryBlockedPlayersCompleteDelegate_Handle(
            FOnQueryBlockedPlayersComplete::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, bool bWasSuccessful, const FString &Error) {
                    this->OnQueryBlockedPlayersComplete.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        bWasSuccessful,
                        Error);
                }));
        this->DelegateHandle_OnRecentPlayersAdded =
            Handle->AddOnRecentPlayersAddedDelegate_Handle(FOnRecentPlayersAdded::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const TArray<TSharedRef<FOnlineRecentPlayer>> &AddedPlayers) {
                    this->OnRecentPlayersAdded.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ConvertArrayElements<TSharedRef<FOnlineRecentPlayer>, UOnlineRecentPlayerRef *>(
                            AddedPlayers,
                            [](const TSharedRef<FOnlineRecentPlayer> &Val) {
                                return UOnlineRecentPlayerRef::FromRecentPlayer(Val);
                            }));
                }));
        this->DelegateHandle_OnFriendSettingsUpdated =
            Handle->AddOnFriendSettingsUpdatedDelegate_Handle(FOnFriendSettingsUpdated::FDelegate::CreateWeakLambda(
                this,
                [this](
                    const FUniqueNetId &UserId,
                    bool bWasSuccessful,
                    bool bWasUpdate,
                    const FFriendSettings &Settings,
                    const FString &ErrorStr) {
                    this->OnFriendSettingsUpdated.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        bWasSuccessful,
                        bWasUpdate,
                        FFriendSettingsData::FromNative(Settings),
                        ErrorStr);
                }));
    }
}
...
UOnlineFriendsSubsystemReadFriendsList *UOnlineFriendsSubsystemReadFriendsList::ReadFriendsList(
    ...
    const FString &ListName)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineFriendsSubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineFriendsSubsystemReadFriendsList::Activate()
{
    ...
    auto __DelegateHandle_OnReadFriendsListComplete = FOnReadFriendsListComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemReadFriendsList::HandleCallback_OnReadFriendsListComplete);

    if (!Handle->ReadFriendsList(
            this->__Store__LocalUserNum,
            this->__Store__ListName,
            __DelegateHandle_OnReadFriendsListComplete))
    {
...
void UOnlineFriendsSubsystemReadFriendsList::HandleCallback_OnReadFriendsListComplete(
    int32 LocalUserNum,
    bool bWasSuccessful,
    const FString &ListName,
    const FString &ErrorStr)
...
UOnlineFriendsSubsystemDeleteFriendsList *UOnlineFriendsSubsystemDeleteFriendsList::DeleteFriendsList(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FString &ListName)
{
...
void UOnlineFriendsSubsystemDeleteFriendsList::Activate()
{
    ...
    auto __DelegateHandle_OnDeleteFriendsListComplete = FOnDeleteFriendsListComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemDeleteFriendsList::HandleCallback_OnDeleteFriendsListComplete);

    if (!Handle->DeleteFriendsList(
            this->__Store__LocalUserNum,
            this->__Store__ListName,
            __DelegateHandle_OnDeleteFriendsListComplete))
    {
...
void UOnlineFriendsSubsystemDeleteFriendsList::HandleCallback_OnDeleteFriendsListComplete(
    int32 LocalUserNum,
    bool bWasSuccessful,
    const FString &ListName,
    const FString &ErrorStr)
...
UOnlineFriendsSubsystemSendInvite *UOnlineFriendsSubsystemSendInvite::SendInvite(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
...
void UOnlineFriendsSubsystemSendInvite::Activate()
{
    ...
    auto __DelegateHandle_OnSendInviteComplete = FOnSendInviteComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemSendInvite::HandleCallback_OnSendInviteComplete);

    if (!Handle->SendInvite(
            this->__Store__LocalUserNum,
            this->__Store__FriendId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__ListName,
            __DelegateHandle_OnSendInviteComplete))
    {
...
void UOnlineFriendsSubsystemSendInvite::HandleCallback_OnSendInviteComplete(
    int32 LocalUserNum,
    bool bWasSuccessful,
    const FUniqueNetId &FriendId,
    const FString &ListName,
    const FString &ErrorStr)
{
    ...
    this->OnSendInviteComplete
        .Broadcast(LocalUserNum, bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(FriendId), ListName, ErrorStr);
}

UOnlineFriendsSubsystemAcceptInvite *UOnlineFriendsSubsystemAcceptInvite::AcceptInvite(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FUniqueNetIdRepl &FriendId,
    const FString &ListName)
...
void UOnlineFriendsSubsystemAcceptInvite::Activate()
{
    ...
    auto __DelegateHandle_OnAcceptInviteComplete = FOnAcceptInviteComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemAcceptInvite::HandleCallback_OnAcceptInviteComplete);

    if (!Handle->AcceptInvite(
            this->__Store__LocalUserNum,
            this->__Store__FriendId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__ListName,
            __DelegateHandle_OnAcceptInviteComplete))
    {
...
void UOnlineFriendsSubsystemAcceptInvite::HandleCallback_OnAcceptInviteComplete(
    int32 LocalUserNum,
    bool bWasSuccessful,
    const FUniqueNetId &FriendId,
    const FString &ListName,
    const FString &ErrorStr)
{
    ...
    this->OnAcceptInviteComplete
        .Broadcast(LocalUserNum, bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(FriendId), ListName, ErrorStr);
}

UOnlineFriendsSubsystemRejectInvite *UOnlineFriendsSubsystemRejectInvite::RejectInvite(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FUniqueNetIdRepl &FriendId,
    const FString &ListName)
...
void UOnlineFriendsSubsystemRejectInvite::Activate()
{
    ...
    this->DelegateHandle_OnRejectInviteComplete = Handle->AddOnRejectInviteCompleteDelegate_Handle(
        this->__Store__LocalUserNum,
        FOnRejectInviteCompleteDelegate::CreateUObject(
            this,
            &UOnlineFriendsSubsystemRejectInvite::HandleCallback_OnRejectInviteComplete));

    if (!Handle->RejectInvite(
            this->__Store__LocalUserNum,
            this->__Store__FriendId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__ListName))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Call 'RejectInvite' did not start successfully"));
        Handle->ClearOnRejectInviteCompleteDelegate_Handle(
            this->__Store__LocalUserNum,
            this->DelegateHandle_OnRejectInviteComplete);
        this->OnCallFailed.Broadcast(false, FUniqueNetIdRepl(), TEXT(""), TEXT(""));
        return;
    }
}

void UOnlineFriendsSubsystemRejectInvite::HandleCallback_OnRejectInviteComplete(
    ...
    const FString &ErrorStr)
{
    ...
    this->OnRejectInviteComplete
        .Broadcast(bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(FriendId), ListName, ErrorStr);
}

UOnlineFriendsSubsystemSetFriendAlias *UOnlineFriendsSubsystemSetFriendAlias::SetFriendAlias(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FUniqueNetIdRepl &FriendId,
    const FString &ListName,
...
void UOnlineFriendsSubsystemSetFriendAlias::Activate()
{
    ...
    auto __DelegateHandle_OnSetFriendAliasComplete = FOnSetFriendAliasComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemSetFriendAlias::HandleCallback_OnSetFriendAliasComplete);

    Handle->SetFriendAlias(
        this->__Store__LocalUserNum,
        this->__Store__FriendId.GetUniqueNetId().ToSharedRef().Get(),
        this->__Store__ListName,
        this->__Store__Alias,
        __DelegateHandle_OnSetFriendAliasComplete);
}

void UOnlineFriendsSubsystemSetFriendAlias::HandleCallback_OnSetFriendAliasComplete(
    int32 LocalUserNum,
    const FUniqueNetId &FriendId,
    const FString &ListName,
    const FOnlineError &Error)
{
    ...
    this->OnSetFriendAliasComplete.Broadcast(
        LocalUserNum,
        ConvertDangerousUniqueNetIdToRepl(FriendId),
        ListName,
        FOnlineErrorInfo::FromNative(Error));
}

UOnlineFriendsSubsystemDeleteFriendAlias *UOnlineFriendsSubsystemDeleteFriendAlias::DeleteFriendAlias(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FUniqueNetIdRepl &FriendId,
    const FString &ListName)
...
void UOnlineFriendsSubsystemDeleteFriendAlias::Activate()
{
    ...
    auto __DelegateHandle_OnDeleteFriendAliasComplete = FOnDeleteFriendAliasComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemDeleteFriendAlias::HandleCallback_OnDeleteFriendAliasComplete);

    Handle->DeleteFriendAlias(
        this->__Store__LocalUserNum,
        this->__Store__FriendId.GetUniqueNetId().ToSharedRef().Get(),
        this->__Store__ListName,
        __DelegateHandle_OnDeleteFriendAliasComplete);
}

void UOnlineFriendsSubsystemDeleteFriendAlias::HandleCallback_OnDeleteFriendAliasComplete(
    int32 LocalUserNum,
    const FUniqueNetId &FriendId,
    const FString &ListName,
    const FOnlineError &Error)
{
    ...
    this->OnDeleteFriendAliasComplete.Broadcast(
        LocalUserNum,
        ConvertDangerousUniqueNetIdToRepl(FriendId),
        ListName,
        FOnlineErrorInfo::FromNative(Error));
...
UOnlineFriendsSubsystemDeleteFriend *UOnlineFriendsSubsystemDeleteFriend::DeleteFriend(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FUniqueNetIdRepl &FriendId,
    const FString &ListName)
...
void UOnlineFriendsSubsystemDeleteFriend::Activate()
{
    ...
    this->DelegateHandle_OnDeleteFriendComplete = Handle->AddOnDeleteFriendCompleteDelegate_Handle(
        this->__Store__LocalUserNum,
        FOnDeleteFriendCompleteDelegate::CreateUObject(
            this,
            &UOnlineFriendsSubsystemDeleteFriend::HandleCallback_OnDeleteFriendComplete));

    if (!Handle->DeleteFriend(
            this->__Store__LocalUserNum,
            this->__Store__FriendId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__ListName))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Call 'DeleteFriend' did not start successfully"));
        Handle->ClearOnDeleteFriendCompleteDelegate_Handle(
            this->__Store__LocalUserNum,
            this->DelegateHandle_OnDeleteFriendComplete);
        this->OnCallFailed.Broadcast(false, FUniqueNetIdRepl(), TEXT(""), TEXT(""));
        return;
    }
}

void UOnlineFriendsSubsystemDeleteFriend::HandleCallback_OnDeleteFriendComplete(
    int32 LocalUserNumCb,
    bool bWasSuccessful,
    const FUniqueNetId &FriendId,
    const FString &ListName,
    const FString &ErrorStr)
{
    ...
    if (LocalUserNumCb != this->__Store__LocalUserNum || FUniqueNetIdRepl(FriendId) != __Store__FriendId)
    {
        // Not the event we're interested in.
        return;
    }
    ...
    this->OnDeleteFriendComplete
        .Broadcast(bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(FriendId), ListName, ErrorStr);
}

bool UOnlineFriendsSubsystem::GetFriendsList(
    int32 LocalUserNum,
    const FString &ListName,
    TArray<UOnlineFriendRef *> &OutFriends)
{
    ...
    OutFriends = ConvertArrayElements<TSharedRef<FOnlineFriend>, UOnlineFriendRef *>(
        __StoreTemp__OutFriends,
        [](const TSharedRef<FOnlineFriend> &Val) {
            return UOnlineFriendRef::FromFriend(Val);
        });

    return __Result;
}

UOnlineFriendRef *UOnlineFriendsSubsystem::GetFriend(
    int32 LocalUserNum,
    const FUniqueNetIdRepl &FriendId,
    const FString &ListName)
{
    ...
    if (!(FriendId.IsValid() && FriendId.GetUniqueNetId().IsValid()))
    {
        return nullptr;
    }

    auto __Result = UOnlineFriendRef::FromFriend(
        Handle->GetFriend(LocalUserNum, FriendId.GetUniqueNetId().ToSharedRef().Get(), ListName));

    return __Result;
}

bool UOnlineFriendsSubsystem::IsFriend(int32 LocalUserNum, const FUniqueNetIdRepl &FriendId, const FString &ListName)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
UOnlineFriendsSubsystemAddRecentPlayers *UOnlineFriendsSubsystemAddRecentPlayers::AddRecentPlayers(
    UOnlineFriendsSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const TArray<FReportPlayedWithUserInfo> &InRecentPlayers,
    const FString &ListName)
{
    if (!IsValid(Subsystem))
    {
...
void UOnlineFriendsSubsystemAddRecentPlayers::Activate()
{
    ...
    if (!(ValidateArrayElements<FReportPlayedWithUserInfo>(
            this->__Store__InRecentPlayers,
            [](const FReportPlayedWithUserInfo &Val) {
                return true;
            })))
    {
    ...
    auto __DelegateHandle_OnAddRecentPlayersComplete = FOnAddRecentPlayersComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemAddRecentPlayers::HandleCallback_OnAddRecentPlayersComplete);

    Handle->AddRecentPlayers(
        this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
        ConvertArrayElements<FReportPlayedWithUserInfo, FReportPlayedWithUser>(
            this->__Store__InRecentPlayers,
            [](const FReportPlayedWithUserInfo &Val) {
                return Val.ToNative();
            }),
        this->__Store__ListName,
        __DelegateHandle_OnAddRecentPlayersComplete);
}

void UOnlineFriendsSubsystemAddRecentPlayers::HandleCallback_OnAddRecentPlayersComplete(
    const FUniqueNetId &UserId,
    const FOnlineError &Error)
{
    if (!IsValid(this->__Store__Subsystem))
    ...
    this->OnAddRecentPlayersComplete.Broadcast(
        ConvertDangerousUniqueNetIdToRepl(UserId),
        FOnlineErrorInfo::FromNative(Error));
}

UOnlineFriendsSubsystemQueryRecentPlayers *UOnlineFriendsSubsystemQueryRecentPlayers::QueryRecentPlayers(
    UOnlineFriendsSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const FString &Namespace)
{
...
void UOnlineFriendsSubsystemQueryRecentPlayers::Activate()
{
    ...
    this->DelegateHandle_OnQueryRecentPlayersComplete =
        Handle->AddOnQueryRecentPlayersCompleteDelegate_Handle(FOnQueryRecentPlayersCompleteDelegate::CreateUObject(
            this,
            &UOnlineFriendsSubsystemQueryRecentPlayers::HandleCallback_OnQueryRecentPlayersComplete));

    if (!Handle->QueryRecentPlayers(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__Namespace))
    {
...
void UOnlineFriendsSubsystemQueryRecentPlayers::HandleCallback_OnQueryRecentPlayersComplete(
    const FUniqueNetId &UserId,
    const FString &Namespace,
    bool bWasSuccessful,
    const FString &Error)
...
bool UOnlineFriendsSubsystem::GetRecentPlayers(
    const FUniqueNetIdRepl &UserId,
    const FString &Namespace,
    TArray<UOnlineRecentPlayerRef *> &OutRecentPlayers)
{
    ...
    auto __Result =
        Handle->GetRecentPlayers(UserId.GetUniqueNetId().ToSharedRef().Get(), Namespace, __StoreTemp__OutRecentPlayers);
    OutRecentPlayers = ConvertArrayElements<TSharedRef<FOnlineRecentPlayer>, UOnlineRecentPlayerRef *>(
        __StoreTemp__OutRecentPlayers,
        [](const TSharedRef<FOnlineRecentPlayer> &Val) {
            return UOnlineRecentPlayerRef::FromRecentPlayer(Val);
        });
...
UOnlineFriendsSubsystemBlockPlayer *UOnlineFriendsSubsystemBlockPlayer::BlockPlayer(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
...
void UOnlineFriendsSubsystemBlockPlayer::Activate()
{
    ...
    this->DelegateHandle_OnBlockedPlayerComplete = Handle->AddOnBlockedPlayerCompleteDelegate_Handle(
        this->__Store__LocalUserNum,
        FOnBlockedPlayerCompleteDelegate::CreateUObject(
            this,
            &UOnlineFriendsSubsystemBlockPlayer::HandleCallback_OnBlockedPlayerComplete));
    ...
    if (!Handle->BlockPlayer(this->__Store__LocalUserNum, this->__Store__PlayerId.GetUniqueNetId().ToSharedRef().Get()))
    {
        ...
        Handle->ClearOnBlockedPlayerCompleteDelegate_Handle(
            this->__Store__LocalUserNum,
            this->DelegateHandle_OnBlockedPlayerComplete);
        this->OnCallFailed.Broadcast(false, FUniqueNetIdRepl(), TEXT(""), TEXT(""));
        return;
    }
}

void UOnlineFriendsSubsystemBlockPlayer::HandleCallback_OnBlockedPlayerComplete(
    int32 LocalUserNumCb,
    bool bWasSuccessful,
    const FUniqueNetId &UniqueId,
    const FString &ListName,
    const FString &ErrorStr)
{
    ...
    if (LocalUserNumCb != this->__Store__LocalUserNum || FUniqueNetIdRepl(UniqueId) != __Store__PlayerId)
    {
        // Not the event we're interested in.
        return;
    }
    ...
    this->OnBlockedPlayerComplete
        .Broadcast(bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(UniqueId), ListName, ErrorStr);
}

UOnlineFriendsSubsystemUnblockPlayer *UOnlineFriendsSubsystemUnblockPlayer::UnblockPlayer(
    UOnlineFriendsSubsystem *Subsystem,
    int32 LocalUserNum,
    const FUniqueNetIdRepl &PlayerId)
{
...
void UOnlineFriendsSubsystemUnblockPlayer::Activate()
{
    ...
    this->DelegateHandle_OnUnblockedPlayerComplete = Handle->AddOnUnblockedPlayerCompleteDelegate_Handle(
        this->__Store__LocalUserNum,
        FOnUnblockedPlayerCompleteDelegate::CreateUObject(
            this,
            &UOnlineFriendsSubsystemUnblockPlayer::HandleCallback_OnUnblockedPlayerComplete));

    if (!Handle->UnblockPlayer(
            this->__Store__LocalUserNum,
            this->__Store__PlayerId.GetUniqueNetId().ToSharedRef().Get()))
    {
        ...
        Handle->ClearOnUnblockedPlayerCompleteDelegate_Handle(
            this->__Store__LocalUserNum,
            this->DelegateHandle_OnUnblockedPlayerComplete);
        this->OnCallFailed.Broadcast(false, FUniqueNetIdRepl(), TEXT(""), TEXT(""));
        return;
    }
}

void UOnlineFriendsSubsystemUnblockPlayer::HandleCallback_OnUnblockedPlayerComplete(
    int32 LocalUserNumCb,
    bool bWasSuccessful,
    const FUniqueNetId &UniqueId,
    const FString &ListName,
    const FString &ErrorStr)
{
    ...
    this->OnUnblockedPlayerComplete
        .Broadcast(bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(UniqueId), ListName, ErrorStr);
}

UOnlineFriendsSubsystemQueryBlockedPlayers *UOnlineFriendsSubsystemQueryBlockedPlayers::QueryBlockedPlayers(
    UOnlineFriendsSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId)
{
    if (!IsValid(Subsystem))
...
void UOnlineFriendsSubsystemQueryBlockedPlayers::Activate()
{
    ...
    this->DelegateHandle_OnQueryBlockedPlayersComplete =
        Handle->AddOnQueryBlockedPlayersCompleteDelegate_Handle(FOnQueryBlockedPlayersCompleteDelegate::CreateUObject(
            this,
            &UOnlineFriendsSubsystemQueryBlockedPlayers::HandleCallback_OnQueryBlockedPlayersComplete));
...
void UOnlineFriendsSubsystemQueryBlockedPlayers::HandleCallback_OnQueryBlockedPlayersComplete(
    const FUniqueNetId &UserId,
    bool bWasSuccessful,
    const FString &Error)
{
...
bool UOnlineFriendsSubsystem::GetBlockedPlayers(
    const FUniqueNetIdRepl &UserId,
    TArray<UOnlineUserRef *> &OutBlockedPlayers)
{
    ...
    auto __Result =
        Handle->GetBlockedPlayers(UserId.GetUniqueNetId().ToSharedRef().Get(), __StoreTemp__OutBlockedPlayers);
    OutBlockedPlayers = ConvertArrayElements<TSharedRef<FOnlineBlockedPlayer>, UOnlineUserRef *>(
        __StoreTemp__OutBlockedPlayers,
        [](const TSharedRef<FOnlineBlockedPlayer> &Val) {
            return UOnlineUserRef::FromBlockedPlayer(Val);
        });
...
UOnlineFriendsSubsystemQueryFriendSettings *UOnlineFriendsSubsystemQueryFriendSettings::QueryFriendSettings(
    UOnlineFriendsSubsystem *Subsystem,
...
void UOnlineFriendsSubsystemQueryFriendSettings::Activate()
{
    ...
    auto __DelegateHandle_OnSettingsOperationComplete = FOnSettingsOperationComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemQueryFriendSettings::HandleCallback_OnSettingsOperationComplete);

    Handle->QueryFriendSettings(
        this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef().Get(),
        __DelegateHandle_OnSettingsOperationComplete);
}

void UOnlineFriendsSubsystemQueryFriendSettings::HandleCallback_OnSettingsOperationComplete(
    const FUniqueNetId &UserId,
    bool bWasSuccessful,
    bool bWasUpdate,
    const FFriendSettings &Settings,
    const FString &ErrorStr)
{
    ...
    this->OnSettingsOperationComplete.Broadcast(
        ConvertDangerousUniqueNetIdToRepl(UserId),
        bWasSuccessful,
        bWasUpdate,
        FFriendSettingsData::FromNative(Settings),
...
bool UOnlineFriendsSubsystem::GetFriendSettings(
    const FUniqueNetIdRepl &UserId,
    TMap<FString, FOnlineFriendSettingsSourceDataConfig> &OutSettings)
{
    ...
    OutSettings = ConvertMapElements<
        FString,
        FString,
        TSharedRef<FOnlineFriendSettingsSourceData>,
        FOnlineFriendSettingsSourceDataConfig>(
        __StoreTemp__OutSettings,
        [](const FString &Val) {
            return Val;
        },
        [](const TSharedRef<FOnlineFriendSettingsSourceData> &Val) {
            return FOnlineFriendSettingsSourceDataConfig::FromNative(Val);
        });

    return __Result;
}

UOnlineFriendsSubsystemSetFriendSettings *UOnlineFriendsSubsystemSetFriendSettings::SetFriendSettings(
    UOnlineFriendsSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
...
void UOnlineFriendsSubsystemSetFriendSettings::Activate()
{
    ...
    auto __DelegateHandle_OnSetFriendSettingsComplete = FOnSetFriendSettingsComplete::CreateUObject(
        this,
        &UOnlineFriendsSubsystemSetFriendSettings::HandleCallback_OnSetFriendSettingsComplete);

    if (!Handle->SetFriendSettings(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__Source,
            this->__Store__bNeverShowAgain,
            __DelegateHandle_OnSetFriendSettingsComplete))
    {
...
void UOnlineFriendsSubsystemSetFriendSettings::HandleCallback_OnSetFriendSettingsComplete(
