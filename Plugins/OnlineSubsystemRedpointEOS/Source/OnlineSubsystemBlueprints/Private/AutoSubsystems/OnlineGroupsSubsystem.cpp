...
FName UOnlineGroupsSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;

TSharedPtr<class IOnlineGroups, ESPMode::ThreadSafe> UOnlineGroupsSubsystem::GetHandle()
{
    ...
    TSharedPtr<class IOnlineGroups, ESPMode::ThreadSafe> Pinned = this->HandleWk.Pin();
    if (Pinned == nullptr || !Pinned.IsValid())
    {
        ...
        auto __Handle__ = Subsystem->GetGroupsInterface();
...
bool UOnlineGroupsSubsystem::IsHandleValid(const TSharedPtr<class IOnlineGroups, ESPMode::ThreadSafe> &InHandle)
{
...
UOnlineGroupsSubsystem::UOnlineGroupsSubsystem()
{
    ...
    this->OSSName = UOnlineGroupsSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineGroupsSubsystem subsystem initialized in context %s"),
    ...
    TSharedPtr<class IOnlineGroups, ESPMode::ThreadSafe> Handle = this->GetHandle();
    if (Handle.IsValid())
    {
...
void UOnlineGroupsSubsystem::BeginDestroy()
{
    if (this->HasAnyFlags(RF_ClassDefaultObject))
    {
        Super::BeginDestroy();
...
void UOnlineGroupsSubsystem::SetNamespace(const FString &Ns)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    ...
    Handle->SetNamespace(Ns);
}

FString UOnlineGroupsSubsystem::GetNamespace()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetNamespace();
