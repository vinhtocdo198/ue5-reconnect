...
FName UOnlineIdentitySubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineIdentitySubsystem::UOnlineIdentitySubsystem()
{
    ...
    this->OSSName = UOnlineIdentitySubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineIdentitySubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
            this->DelegateHandle_OnLoginStatusChanged.Add(Handle->AddOnLoginStatusChangedDelegate_Handle(
                i,
                FOnLoginStatusChanged::FDelegate::CreateWeakLambda(
                    this,
                    [this,
                    ...
                    [this,
                     i](int32 _UnusedLocalUserNum,
                        ...
                        this->OnLoginComplete
                            .Broadcast(i, bWasSuccessful, ConvertDangerousUniqueNetIdToRepl(UserId), Error);
                    })));
            this->DelegateHandle_OnLogoutComplete.Add(Handle->AddOnLogoutCompleteDelegate_Handle(
                i,
                FOnLogoutComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this, i](int32 _UnusedLocalUserNum, bool bWasSuccessful) {
        ...
        this->DelegateHandle_OnLoginFlowLogout = Handle->AddOnLoginFlowLogoutDelegate_Handle(
            FOnLoginFlowLogout::FDelegate::CreateWeakLambda(this, [this](const TArray<FString> &LoginDomains) {
                this->OnLoginFlowLogout.Broadcast(
                    ConvertArrayElements<FString, FString>(LoginDomains, [](const FString &Val) {
                        return Val;
                    }));
            }));
    }
...
UOnlineIdentitySubsystemLogin *UOnlineIdentitySubsystemLogin::Login(
    ...
    const FOnlineAccountCredential &AccountCredentials)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineIdentitySubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineIdentitySubsystemLogin::Activate()
{
    ...
    this->DelegateHandle_OnLoginComplete = Handle->AddOnLoginCompleteDelegate_Handle(
...
void UOnlineIdentitySubsystemLogin::HandleCallback_OnLoginComplete(
...
UOnlineIdentitySubsystemLogout *UOnlineIdentitySubsystemLogout::Logout(
    UOnlineIdentitySubsystem *Subsystem,
    int32 LocalUserNum)
{
    if (!IsValid(Subsystem))
...
void UOnlineIdentitySubsystemLogout::Activate()
{
    ...
    this->DelegateHandle_OnLogoutComplete = Handle->AddOnLogoutCompleteDelegate_Handle(
        this->__Store__LocalUserNum,
        FOnLogoutCompleteDelegate::CreateUObject(
            this,
            &UOnlineIdentitySubsystemLogout::HandleCallback_OnLogoutComplete));

    if (!Handle->Logout(this->__Store__LocalUserNum))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Call 'Logout' did not start successfully"));
        Handle->ClearOnLogoutCompleteDelegate_Handle(
            this->__Store__LocalUserNum,
            this->DelegateHandle_OnLogoutComplete);
        this->OnCallFailed.Broadcast(false);
        return;
    }
}
...
UOnlineIdentitySubsystemAutoLogin *UOnlineIdentitySubsystemAutoLogin::AutoLogin(
    UOnlineIdentitySubsystem *Subsystem,
    int32 LocalUserNum)
{
    if (!IsValid(Subsystem))
...
void UOnlineIdentitySubsystemAutoLogin::Activate()
{
    ...
    this->DelegateHandle_OnLoginComplete = Handle->AddOnLoginCompleteDelegate_Handle(
        ...
        FOnLoginCompleteDelegate::CreateUObject(
            this,
            &UOnlineIdentitySubsystemAutoLogin::HandleCallback_OnLoginComplete));
...
void UOnlineIdentitySubsystemAutoLogin::HandleCallback_OnLoginComplete(
    int32 LocalUserNumCb,
    bool bWasSuccessful,
    const FUniqueNetId &UserId,
    const FString &Error)
...
UUserOnlineAccountRef *UOnlineIdentitySubsystem::GetUserAccount(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    ...
    auto __Result = UUserOnlineAccountRef::FromUserOnlineAccount(
        Handle->GetUserAccount(UserId.GetUniqueNetId().ToSharedRef().Get()));
...
TArray<UUserOnlineAccountRef *> UOnlineIdentitySubsystem::GetAllUserAccounts()
{
    ...
    auto __Result = ConvertArrayElements<TSharedPtr<FUserOnlineAccount>, UUserOnlineAccountRef *>(
        Handle->GetAllUserAccounts(),
        [](const TSharedPtr<FUserOnlineAccount> &Val) {
            return UUserOnlineAccountRef::FromUserOnlineAccount(Val);
        });
...
FUniqueNetIdRepl UOnlineIdentitySubsystem::CreateUniquePlayerId(const FString &Str)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
FString UOnlineIdentitySubsystem::GetPlayerNickname(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
UOnlineIdentitySubsystemRevokeAuthToken *UOnlineIdentitySubsystemRevokeAuthToken::RevokeAuthToken(
    UOnlineIdentitySubsystem *Subsystem,
...
void UOnlineIdentitySubsystemRevokeAuthToken::Activate()
{
    ...
    auto __DelegateHandle_OnRevokeAuthTokenComplete = FOnRevokeAuthTokenCompleteDelegate::CreateUObject(
        this,
        &UOnlineIdentitySubsystemRevokeAuthToken::HandleCallback_OnRevokeAuthTokenComplete);

    Handle->RevokeAuthToken(
        this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef().Get(),
        __DelegateHandle_OnRevokeAuthTokenComplete);
}

void UOnlineIdentitySubsystemRevokeAuthToken::HandleCallback_OnRevokeAuthTokenComplete(
    const FUniqueNetId &UserId,
    const FOnlineError &OnlineError)
{
    ...
    this->OnRevokeAuthTokenComplete.Broadcast(
        ConvertDangerousUniqueNetIdToRepl(UserId),
        FOnlineErrorInfo::FromNative(OnlineError));
}

void UOnlineIdentitySubsystem::ClearCachedAuthToken(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    Handle->ClearCachedAuthToken(UserId.GetUniqueNetId().ToSharedRef().Get());
}

UOnlineIdentitySubsystemGetUserPrivilege *UOnlineIdentitySubsystemGetUserPrivilege::GetUserPrivilege(
    UOnlineIdentitySubsystem *Subsystem,
    const FUniqueNetIdRepl &LocalUserId,
    ...
    EShowPrivilegeResolveUI_ ShowResolveUI)
{
    ...
    Node->__Store__ShowResolveUI = ShowResolveUI;
    return Node;
}
...
void UOnlineIdentitySubsystemGetUserPrivilege::Activate()
{
    ...
    auto __DelegateHandle_OnGetUserPrivilegeComplete =
        IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate::CreateUObject(
            this,
            &UOnlineIdentitySubsystemGetUserPrivilege::HandleCallback_OnGetUserPrivilegeComplete);

    Handle->GetUserPrivilege(
        this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef().Get(),
        (EUserPrivileges::Type)(this->__Store__Privilege),
        __DelegateHandle_OnGetUserPrivilegeComplete
#if REDPOINT_EOS_UE_5_4_OR_LATER
        ,
        (EShowPrivilegeResolveUI)(this->__Store__ShowResolveUI)
#endif
    );
}

void UOnlineIdentitySubsystemGetUserPrivilege::HandleCallback_OnGetUserPrivilegeComplete(
    ...
    uint32 PrivilegeResult)
{
    ...
    this->OnGetUserPrivilegeComplete.Broadcast(
        ConvertDangerousUniqueNetIdToRepl(LocalUserId),
        (EOnlineUserPrivilege)(Privilege),
        (int64)(PrivilegeResult));
}

int32 UOnlineIdentitySubsystem::GetPlatformUserIdFromUniqueNetId(const FUniqueNetIdRepl &UniqueNetId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = PlatformUserIdToBlueprintType(
        Handle->GetPlatformUserIdFromUniqueNetId(UniqueNetId.GetUniqueNetId().ToSharedRef().Get()));
