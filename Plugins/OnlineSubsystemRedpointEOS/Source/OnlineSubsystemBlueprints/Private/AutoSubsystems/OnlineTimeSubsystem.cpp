...
FName UOnlineTimeSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;

TSharedPtr<class IOnlineTime, ESPMode::ThreadSafe> UOnlineTimeSubsystem::GetHandle()
{
    ...
    TSharedPtr<class IOnlineTime, ESPMode::ThreadSafe> Pinned = this->HandleWk.Pin();
    if (Pinned == nullptr || !Pinned.IsValid())
    {
        ...
        auto __Handle__ = Subsystem->GetTimeInterface();
...
bool UOnlineTimeSubsystem::IsHandleValid(const TSharedPtr<class IOnlineTime, ESPMode::ThreadSafe> &InHandle)
{
    return InHandle.IsValid();
}

UOnlineTimeSubsystem::UOnlineTimeSubsystem()
{
    ...
    this->OSSName = UOnlineTimeSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineTimeSubsystem subsystem initialized in context %s"),
    ...
    TSharedPtr<class IOnlineTime, ESPMode::ThreadSafe> Handle = this->GetHandle();
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
        }
        this->DelegateHandle_OnQueryServerUtcTimeComplete = Handle->AddOnQueryServerUtcTimeCompleteDelegate_Handle(
            FOnQueryServerUtcTimeComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool Param1, const FString &Param2, const FString &Param3) {
                    this->OnQueryServerUtcTimeComplete.Broadcast(Param1, Param2, Param3);
                }));
    }
}

void UOnlineTimeSubsystem::BeginDestroy()
{
    if (this->HasAnyFlags(RF_ClassDefaultObject))
    {
        Super::BeginDestroy();
    ...
    if (Handle.IsValid())
    {
        ...
        Handle->ClearOnQueryServerUtcTimeCompleteDelegate_Handle(this->DelegateHandle_OnQueryServerUtcTimeComplete);
    }
...
bool UOnlineTimeSubsystem::QueryServerUtcTime()
{
    ...
    auto __Result = Handle->QueryServerUtcTime();

    return __Result;
}

FString UOnlineTimeSubsystem::GetLastServerUtcTime()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetLastServerUtcTime();
