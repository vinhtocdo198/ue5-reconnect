...
FName UOnlineLobbySubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineLobbySubsystem::UOnlineLobbySubsystem()
{
    ...
    this->OSSName = UOnlineLobbySubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineLobbySubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnLobbyUpdate =
            Handle->AddOnLobbyUpdateDelegate_Handle(FOnLobbyUpdate::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FOnlineLobbyId &LobbyId) {
                    this->OnLobbyUpdate.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ULobbyId::FromNative(LobbyId));
                }));
        this->DelegateHandle_OnLobbyDelete =
            Handle->AddOnLobbyDeleteDelegate_Handle(FOnLobbyDelete::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FOnlineLobbyId &LobbyId) {
                    this->OnLobbyDelete.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ULobbyId::FromNative(LobbyId));
                }));
        this->DelegateHandle_OnMemberConnect =
            Handle->AddOnMemberConnectDelegate_Handle(FOnMemberConnect::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FOnlineLobbyId &LobbyId, const FUniqueNetId &MemberId) {
                    this->OnMemberConnect.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ULobbyId::FromNative(LobbyId),
                        ConvertDangerousUniqueNetIdToRepl(MemberId));
                }));
        this->DelegateHandle_OnMemberUpdate =
            Handle->AddOnMemberUpdateDelegate_Handle(FOnMemberUpdate::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &UserId, const FOnlineLobbyId &LobbyId, const FUniqueNetId &MemberId) {
                    this->OnMemberUpdate.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ULobbyId::FromNative(LobbyId),
                        ConvertDangerousUniqueNetIdToRepl(MemberId));
                }));
        this->DelegateHandle_OnMemberDisconnect =
            Handle->AddOnMemberDisconnectDelegate_Handle(FOnMemberDisconnect::FDelegate::CreateWeakLambda(
                this,
                [this](
                    ...
                    bool bWasKicked) {
                    this->OnMemberDisconnect.Broadcast(
                        ConvertDangerousUniqueNetIdToRepl(UserId),
                        ULobbyId::FromNative(LobbyId),
                        ConvertDangerousUniqueNetIdToRepl(MemberId),
                        bWasKicked);
...
UOnlineLobbyTransaction *UOnlineLobbySubsystem::MakeCreateLobbyTransaction(const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    ...
    auto __Result = UOnlineLobbyTransaction::FromNative(
        Handle->MakeCreateLobbyTransaction(UserId.GetUniqueNetId().ToSharedRef().Get()));

    return __Result;
}

UOnlineLobbyTransaction *UOnlineLobbySubsystem::MakeUpdateLobbyTransaction(
    ...
    ULobbyId *LobbyId)
{
    ...
    auto __Result = UOnlineLobbyTransaction::FromNative(
        Handle->MakeUpdateLobbyTransaction(UserId.GetUniqueNetId().ToSharedRef().Get(), LobbyId->ToNative()));

    return __Result;
}

UOnlineLobbyMemberTransaction *UOnlineLobbySubsystem::MakeUpdateLobbyMemberTransaction(
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
    const FUniqueNetIdRepl &MemberId)
{
    ...
    auto __Result = UOnlineLobbyMemberTransaction::FromNative(Handle->MakeUpdateLobbyMemberTransaction(
...
UOnlineLobbySubsystemCreateLobby *UOnlineLobbySubsystemCreateLobby::CreateLobby(
    ...
    UOnlineLobbyTransaction *Transaction)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineLobbySubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineLobbySubsystemCreateLobby::Activate()
{
    ...
    auto __DelegateHandle_OnLobbyCreateOrConnectComplete = FOnLobbyCreateOrConnectComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemCreateLobby::HandleCallback_OnLobbyCreateOrConnectComplete);

    if (!Handle->CreateLobby(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            *this->__Store__Transaction->ToNative(),
            __DelegateHandle_OnLobbyCreateOrConnectComplete))
    {
...
void UOnlineLobbySubsystemCreateLobby::HandleCallback_OnLobbyCreateOrConnectComplete(
    ...
    const TSharedPtr<class FOnlineLobby> &Lobby)
{
    ...
    this->OnLobbyCreateOrConnectComplete.Broadcast(
        FOnlineErrorInfo::FromNative(Error),
        ConvertDangerousUniqueNetIdToRepl(UserId),
        ULobby::FromNative(Lobby));
}

UOnlineLobbySubsystemUpdateLobby *UOnlineLobbySubsystemUpdateLobby::UpdateLobby(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
    UOnlineLobbyTransaction *Transaction)
...
void UOnlineLobbySubsystemUpdateLobby::Activate()
{
    ...
    auto __DelegateHandle_OnLobbyOperationComplete = FOnLobbyOperationComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemUpdateLobby::HandleCallback_OnLobbyOperationComplete);

    if (!Handle->UpdateLobby(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__LobbyId->ToNative(),
            *this->__Store__Transaction->ToNative(),
            __DelegateHandle_OnLobbyOperationComplete))
    {
...
void UOnlineLobbySubsystemUpdateLobby::HandleCallback_OnLobbyOperationComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId)
{
    if (!IsValid(this->__Store__Subsystem))
    ...
    this->OnLobbyOperationComplete.Broadcast(
        FOnlineErrorInfo::FromNative(Error),
        ConvertDangerousUniqueNetIdToRepl(UserId));
}

UOnlineLobbySubsystemDeleteLobby *UOnlineLobbySubsystemDeleteLobby::DeleteLobby(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId)
{
...
void UOnlineLobbySubsystemDeleteLobby::Activate()
{
    ...
    auto __DelegateHandle_OnLobbyOperationComplete = FOnLobbyOperationComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemDeleteLobby::HandleCallback_OnLobbyOperationComplete);

    if (!Handle->DeleteLobby(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__LobbyId->ToNative(),
            __DelegateHandle_OnLobbyOperationComplete))
    {
...
void UOnlineLobbySubsystemDeleteLobby::HandleCallback_OnLobbyOperationComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId)
{
    if (!IsValid(this->__Store__Subsystem))
...
UOnlineLobbySubsystemConnectLobby *UOnlineLobbySubsystemConnectLobby::ConnectLobby(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId)
{
...
void UOnlineLobbySubsystemConnectLobby::Activate()
{
    ...
    auto __DelegateHandle_OnLobbyCreateOrConnectComplete = FOnLobbyCreateOrConnectComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemConnectLobby::HandleCallback_OnLobbyCreateOrConnectComplete);

    if (!Handle->ConnectLobby(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__LobbyId->ToNative(),
            __DelegateHandle_OnLobbyCreateOrConnectComplete))
    {
...
void UOnlineLobbySubsystemConnectLobby::HandleCallback_OnLobbyCreateOrConnectComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId,
    const TSharedPtr<class FOnlineLobby> &Lobby)
{
...
UOnlineLobbySubsystemDisconnectLobby *UOnlineLobbySubsystemDisconnectLobby::DisconnectLobby(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId)
{
...
void UOnlineLobbySubsystemDisconnectLobby::Activate()
{
    ...
    auto __DelegateHandle_OnLobbyOperationComplete = FOnLobbyOperationComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemDisconnectLobby::HandleCallback_OnLobbyOperationComplete);

    if (!Handle->DisconnectLobby(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__LobbyId->ToNative(),
            __DelegateHandle_OnLobbyOperationComplete))
    {
...
void UOnlineLobbySubsystemDisconnectLobby::HandleCallback_OnLobbyOperationComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId)
{
    if (!IsValid(this->__Store__Subsystem))
...
UOnlineLobbySubsystemUpdateMemberSelf *UOnlineLobbySubsystemUpdateMemberSelf::UpdateMemberSelf(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
...
void UOnlineLobbySubsystemUpdateMemberSelf::Activate()
{
    ...
    auto __DelegateHandle_OnLobbyOperationComplete = FOnLobbyOperationComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemUpdateMemberSelf::HandleCallback_OnLobbyOperationComplete);

    if (!Handle->UpdateMemberSelf(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__LobbyId->ToNative(),
            *this->__Store__Transaction->ToNative(),
            __DelegateHandle_OnLobbyOperationComplete))
...
void UOnlineLobbySubsystemUpdateMemberSelf::HandleCallback_OnLobbyOperationComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId)
{
    if (!IsValid(this->__Store__Subsystem))
...
bool UOnlineLobbySubsystem::GetMemberCount(const FUniqueNetIdRepl &UserId, ULobbyId *LobbyId, int32 &OutMemberCount)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetMemberCount(
        UserId.GetUniqueNetId().ToSharedRef().Get(),
        LobbyId->ToNative(),
        __StoreTemp__OutMemberCount);
    OutMemberCount = __StoreTemp__OutMemberCount;

    return __Result;
}

bool UOnlineLobbySubsystem::GetMemberUserId(
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
    int32 MemberIndex,
    FUniqueNetIdRepl &OutMemberId)
{
    ...
    auto __Result = Handle->GetMemberUserId(
        UserId.GetUniqueNetId().ToSharedRef().Get(),
        LobbyId->ToNative(),
        MemberIndex,
        __StoreTemp__OutMemberId);
    OutMemberId = FUniqueNetIdRepl(__StoreTemp__OutMemberId);

    return __Result;
}

bool UOnlineLobbySubsystem::GetMemberMetadataValue(
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
    const FUniqueNetIdRepl &MemberId,
    ...
    FVariantDataBP &OutMetadataValue)
{
    ...
    auto __Result = Handle->GetMemberMetadataValue(
        UserId.GetUniqueNetId().ToSharedRef().Get(),
        LobbyId->ToNative(),
        MemberId.GetUniqueNetId().ToSharedRef().Get(),
        MetadataKey,
        __StoreTemp__OutMetadataValue);
    OutMetadataValue = FVariantDataBP::FromNative(__StoreTemp__OutMetadataValue);

    return __Result;
}

UOnlineLobbySubsystemSearch *UOnlineLobbySubsystemSearch::Search(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    const FOnlineLobbySearchQueryBP &Query)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
...
void UOnlineLobbySubsystemSearch::Activate()
{
    ...
    auto __DelegateHandle_OnLobbySearchComplete =
        FOnLobbySearchComplete::CreateUObject(this, &UOnlineLobbySubsystemSearch::HandleCallback_OnLobbySearchComplete);

    if (!Handle->Search(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__Query.ToNative(),
            __DelegateHandle_OnLobbySearchComplete))
    {
...
void UOnlineLobbySubsystemSearch::HandleCallback_OnLobbySearchComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId,
    const TArray<TSharedRef<const FOnlineLobbyId>> &Lobbies)
{
    ...
    this->OnLobbySearchComplete.Broadcast(
        FOnlineErrorInfo::FromNative(Error),
        ConvertDangerousUniqueNetIdToRepl(UserId),
        ConvertArrayElements<TSharedRef<const FOnlineLobbyId>, ULobbyId *>(
            Lobbies,
            [](const TSharedRef<const FOnlineLobbyId> &Val) {
                return ULobbyId::FromNative(Val);
            }));
}

bool UOnlineLobbySubsystem::GetLobbyMetadataValue(
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
    const FString &MetadataKey,
    FVariantDataBP &OutMetadataValue)
{
    ...
    auto __Result = Handle->GetLobbyMetadataValue(
        UserId.GetUniqueNetId().ToSharedRef().Get(),
        LobbyId->ToNative(),
        MetadataKey,
        __StoreTemp__OutMetadataValue);
...
ULobbyId *UOnlineLobbySubsystem::ParseSerializedLobbyId(const FString &InLobbyId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
UOnlineLobbySubsystemKickMember *UOnlineLobbySubsystemKickMember::KickMember(
    UOnlineLobbySubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
    ULobbyId *LobbyId,
    const FUniqueNetIdRepl &MemberId)
...
void UOnlineLobbySubsystemKickMember::Activate()
{
    ...
    if (!(this->__Store__MemberId.IsValid() && this->__Store__MemberId.GetUniqueNetId().IsValid()))
    {
    ...
    auto __DelegateHandle_OnLobbyOperationComplete = FOnLobbyOperationComplete::CreateUObject(
        this,
        &UOnlineLobbySubsystemKickMember::HandleCallback_OnLobbyOperationComplete);

    if (!Handle->KickMember(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__LobbyId->ToNative(),
            this->__Store__MemberId.GetUniqueNetId().ToSharedRef().Get(),
            __DelegateHandle_OnLobbyOperationComplete))
    {
...
void UOnlineLobbySubsystemKickMember::HandleCallback_OnLobbyOperationComplete(
    const FOnlineError &Error,
    const FUniqueNetId &UserId)
{
    if (!IsValid(this->__Store__Subsystem))
