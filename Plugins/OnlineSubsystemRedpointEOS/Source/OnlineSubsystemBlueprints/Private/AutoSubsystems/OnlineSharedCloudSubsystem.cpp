...
bool UOnlineSharedCloudSubsystem::IsHandleValid(
    const TSharedPtr<class IOnlineSharedCloud, ESPMode::ThreadSafe> &InHandle)
{
...
UOnlineSharedCloudSubsystem::UOnlineSharedCloudSubsystem()
{
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineSharedCloudSubsystem subsystem initialized in context %s"),
...
bool UOnlineSharedCloudSubsystem::WriteSharedFile(
    ...
    UFileData *&Contents)
{
    ...
    auto __Result =
        Handle->WriteSharedFile(UserId.GetUniqueNetId().ToSharedRef().Get(), Filename, __StoreTemp__Contents);
