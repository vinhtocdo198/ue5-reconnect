...
FName UOnlineTitleFileSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineTitleFileSubsystem::UOnlineTitleFileSubsystem()
{
    ...
    this->OSSName = UOnlineTitleFileSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineTitleFileSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnEnumerateFilesComplete =
            Handle->AddOnEnumerateFilesCompleteDelegate_Handle(FOnEnumerateFilesComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FString &Error) {
                    this->OnEnumerateFilesComplete.Broadcast(bWasSuccessful, Error);
                }));
        this->DelegateHandle_OnReadFileComplete =
            Handle->AddOnReadFileCompleteDelegate_Handle(FOnReadFileComplete::FDelegate::CreateWeakLambda(
                this,
                [this](bool bWasSuccessful, const FString &FileName) {
...
bool UOnlineTitleFileSubsystem::GetFileContents(const FString &FileName, UFileData *&FileContents)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
...
bool UOnlineTitleFileSubsystem::ClearFile(const FString &FileName)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
UOnlineTitleFileSubsystemEnumerateFiles *UOnlineTitleFileSubsystemEnumerateFiles::EnumerateFiles(
    UOnlineTitleFileSubsystem *Subsystem,
    const FPagedQueryBP &Page)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineTitleFileSubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineTitleFileSubsystemEnumerateFiles::Activate()
{
    ...
    this->DelegateHandle_OnEnumerateFilesComplete =
        Handle->AddOnEnumerateFilesCompleteDelegate_Handle(FOnEnumerateFilesCompleteDelegate::CreateUObject(
            this,
            &UOnlineTitleFileSubsystemEnumerateFiles::HandleCallback_OnEnumerateFilesComplete));
...
void UOnlineTitleFileSubsystemEnumerateFiles::HandleCallback_OnEnumerateFilesComplete(
    bool bWasSuccessful,
    const FString &Error)
{
...
void UOnlineTitleFileSubsystem::GetFileList(TArray<FCloudFileHeaderBP> &Files)
{
    ...
    Files =
        ConvertArrayElements<FCloudFileHeader, FCloudFileHeaderBP>(__StoreTemp__Files, [](const FCloudFileHeader &Val) {
            return FCloudFileHeaderBP::FromNative(Val);
        });
}

UOnlineTitleFileSubsystemReadFile *UOnlineTitleFileSubsystemReadFile::ReadFile(
    UOnlineTitleFileSubsystem *Subsystem,
...
void UOnlineTitleFileSubsystemReadFile::Activate()
{
    ...
    this->DelegateHandle_OnReadFileComplete =
        Handle->AddOnReadFileCompleteDelegate_Handle(FOnReadFileCompleteDelegate::CreateUObject(
            this,
            &UOnlineTitleFileSubsystemReadFile::HandleCallback_OnReadFileComplete));
    this->ProgressDelegateHandle_OnReadFileComplete =
        Handle->AddOnReadFileProgressDelegate_Handle(FOnReadFileProgressDelegate::CreateUObject(
            this,
            &UOnlineTitleFileSubsystemReadFile::HandleCallbackProgress_OnReadFileComplete));
...
void UOnlineTitleFileSubsystemReadFile::HandleCallbackProgress_OnReadFileComplete(
    const FString &FileName,
    uint64 NumBytes)
{
    if (!IsValid(this->__Store__Subsystem))
