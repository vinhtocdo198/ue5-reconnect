...
FName UOnlineMessageSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineMessageSubsystem::UOnlineMessageSubsystem()
{
    ...
    this->OSSName = UOnlineMessageSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineMessageSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
            this->DelegateHandle_OnEnumerateMessagesComplete.Add(Handle->AddOnEnumerateMessagesCompleteDelegate_Handle(
                i,
                FOnEnumerateMessagesComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this, i](int32 _UnusedLocalUserNum, bool Param1, const FString &Param2) {
            ...
            this->DelegateHandle_OnSendMessageComplete.Add(Handle->AddOnSendMessageCompleteDelegate_Handle(
                i,
                FOnSendMessageComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this, i](int32 _UnusedLocalUserNum, bool Param1, const FString &Param2) {
...
void UOnlineMessageSubsystem::BeginDestroy()
{
    ...
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
            Handle->ClearOnEnumerateMessagesCompleteDelegate_Handle(
                i,
                this->DelegateHandle_OnEnumerateMessagesComplete[i]);
