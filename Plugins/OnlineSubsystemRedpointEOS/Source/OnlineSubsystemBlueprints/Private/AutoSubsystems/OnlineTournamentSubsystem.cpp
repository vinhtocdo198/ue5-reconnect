...
FName UOnlineTournamentSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;

TSharedPtr<class IOnlineTournament, ESPMode::ThreadSafe> UOnlineTournamentSubsystem::GetHandle()
{
    ...
    TSharedPtr<class IOnlineTournament, ESPMode::ThreadSafe> Pinned = this->HandleWk.Pin();
    if (Pinned == nullptr || !Pinned.IsValid())
    {
        ...
        auto __Handle__ = Subsystem->GetTournamentInterface();
...
bool UOnlineTournamentSubsystem::IsHandleValid(const TSharedPtr<class IOnlineTournament, ESPMode::ThreadSafe> &InHandle)
{
...
UOnlineTournamentSubsystem::UOnlineTournamentSubsystem()
{
    ...
    this->OSSName = UOnlineTournamentSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineTournamentSubsystem subsystem initialized in context %s"),
    ...
    TSharedPtr<class IOnlineTournament, ESPMode::ThreadSafe> Handle = this->GetHandle();
    if (Handle.IsValid())
    {
...
void UOnlineTournamentSubsystem::BeginDestroy()
{
    if (this->HasAnyFlags(RF_ClassDefaultObject))
    {
        Super::BeginDestroy();
