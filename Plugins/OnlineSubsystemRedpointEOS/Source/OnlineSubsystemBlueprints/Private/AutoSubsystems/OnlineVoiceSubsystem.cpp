...
FName UOnlineVoiceSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineVoiceSubsystem::UOnlineVoiceSubsystem()
{
    ...
    this->OSSName = UOnlineVoiceSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineVoiceSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnPlayerTalkingStateChanged = Handle->AddOnPlayerTalkingStateChangedDelegate_Handle(
            FOnPlayerTalkingStateChanged::FDelegate::CreateWeakLambda(
                this,
                [this](const FUniqueNetIdRef &Param1, bool Param2) {
...
bool UOnlineVoiceSubsystem::RegisterRemoteTalker(const FUniqueNetIdRepl &UniqueId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineVoiceSubsystem::UnregisterRemoteTalker(const FUniqueNetIdRepl &UniqueId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineVoiceSubsystem::IsRemotePlayerTalking(const FUniqueNetIdRepl &UniqueId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineVoiceSubsystem::IsMuted(int64 LocalUserNum, const FUniqueNetIdRepl &UniqueId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineVoiceSubsystem::MuteRemoteTalker(int32 LocalUserNum, const FUniqueNetIdRepl &PlayerId, bool bIsSystemWide)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result =
        Handle->MuteRemoteTalker((uint8)(LocalUserNum), PlayerId.GetUniqueNetId().ToSharedRef().Get(), bIsSystemWide);

    return __Result;
}

bool UOnlineVoiceSubsystem::UnmuteRemoteTalker(int32 LocalUserNum, const FUniqueNetIdRepl &PlayerId, bool bIsSystemWide)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result =
        Handle->UnmuteRemoteTalker((uint8)(LocalUserNum), PlayerId.GetUniqueNetId().ToSharedRef().Get(), bIsSystemWide);
...
float UOnlineVoiceSubsystem::GetAmplitudeOfRemoteTalker(const FUniqueNetIdRepl &PlayerId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineVoiceSubsystem::PatchRemoteTalkerOutputToEndpoint(const FString &InDeviceName, bool bMuteInGameOutput)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UOnlineVoiceSubsystem::PatchLocalTalkerOutputToEndpoint(const FString &InDeviceName)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
