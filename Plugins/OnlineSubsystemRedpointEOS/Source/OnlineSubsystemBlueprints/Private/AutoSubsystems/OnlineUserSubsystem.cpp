...
FName UOnlineUserSubsystem::OSSNameToAssignDuringConstruction = NAME_Default;
...
UOnlineUserSubsystem::UOnlineUserSubsystem()
{
    ...
    this->OSSName = UOnlineUserSubsystem::OSSNameToAssignDuringConstruction;
    ...
    UE_LOG(
        LogOnlineBlueprint,
        Verbose,
        TEXT("OnlineUserSubsystem subsystem initialized in context %s"),
    ...
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
            this->DelegateHandle_OnQueryUserInfoComplete.Add(Handle->AddOnQueryUserInfoCompleteDelegate_Handle(
                i,
                FOnQueryUserInfoComplete::FDelegate::CreateWeakLambda(
                    this,
                    [this,
                     i](int32 _UnusedLocalUserNum,
                        ...
                        this->OnQueryUserInfoComplete.Broadcast(
                            i,
                            bWasSuccessful,
...
UOnlineUserSubsystemQueryUserInfo *UOnlineUserSubsystemQueryUserInfo::QueryUserInfo(
    ...
    const TArray<FUniqueNetIdRepl> &UserIds)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(
            LogOnlineBlueprint,
            Error,
            TEXT("OnlineUserSubsystem is not available. It's possible the online subsystem you are using doesn't "
...
void UOnlineUserSubsystemQueryUserInfo::Activate()
{
    ...
    if (!(ValidateArrayElements<FUniqueNetIdRepl>(this->__Store__UserIds, [](const FUniqueNetIdRepl &Val) {
    ...
    this->DelegateHandle_OnQueryUserInfoComplete = Handle->AddOnQueryUserInfoCompleteDelegate_Handle(
        this->__Store__LocalUserNum,
        FOnQueryUserInfoCompleteDelegate::CreateUObject(
            this,
            &UOnlineUserSubsystemQueryUserInfo::HandleCallback_OnQueryUserInfoComplete));

    if (!Handle->QueryUserInfo(
            this->__Store__LocalUserNum,
            ConvertArrayElements<FUniqueNetIdRepl, FUniqueNetIdRef>(
                this->__Store__UserIds,
                ...
                })))
    {
        ...
        Handle->ClearOnQueryUserInfoCompleteDelegate_Handle(
            this->__Store__LocalUserNum,
            this->DelegateHandle_OnQueryUserInfoComplete);
        this->OnCallFailed.Broadcast(false, TArray<FUniqueNetIdRepl>(), TEXT(""));
        return;
    }
}

void UOnlineUserSubsystemQueryUserInfo::HandleCallback_OnQueryUserInfoComplete(
...
bool UOnlineUserSubsystem::GetAllUserInfo(int32 LocalUserNum, TArray<UOnlineUserRef *> &OutUsers)
{
    ...
    OutUsers = ConvertArrayElements<TSharedRef<class FOnlineUser>, UOnlineUserRef *>(
        __StoreTemp__OutUsers,
        [](const TSharedRef<class FOnlineUser> &Val) {
            return UOnlineUserRef::FromUser(Val);
        });

    return __Result;
}

UOnlineUserRef *UOnlineUserSubsystem::GetUserInfo(int32 LocalUserNum, const FUniqueNetIdRepl &UserId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result =
        UOnlineUserRef::FromUser(Handle->GetUserInfo(LocalUserNum, UserId.GetUniqueNetId().ToSharedRef().Get()));

    return __Result;
}

UOnlineUserSubsystemQueryUserIdMapping *UOnlineUserSubsystemQueryUserIdMapping::QueryUserIdMapping(
    UOnlineUserSubsystem *Subsystem,
...
void UOnlineUserSubsystemQueryUserIdMapping::Activate()
{
    ...
    auto __DelegateHandle_OnQueryUserMappingComplete = IOnlineUser::FOnQueryUserMappingComplete::CreateUObject(
        this,
        &UOnlineUserSubsystemQueryUserIdMapping::HandleCallback_OnQueryUserMappingComplete);

    if (!Handle->QueryUserIdMapping(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__DisplayNameOrEmail,
            __DelegateHandle_OnQueryUserMappingComplete))
    {
...
void UOnlineUserSubsystemQueryUserIdMapping::HandleCallback_OnQueryUserMappingComplete(
    bool bWasSuccessful,
    ...
    const FString &Error)
{
    ...
    this->OnQueryUserMappingComplete.Broadcast(
        bWasSuccessful,
        ConvertDangerousUniqueNetIdToRepl(UserId),
        DisplayNameOrEmail,
        ConvertDangerousUniqueNetIdToRepl(FoundUserId),
        Error);
}

UOnlineUserSubsystemQueryExternalIdMappings *UOnlineUserSubsystemQueryExternalIdMappings::QueryExternalIdMappings(
    UOnlineUserSubsystem *Subsystem,
    const FUniqueNetIdRepl &UserId,
...
void UOnlineUserSubsystemQueryExternalIdMappings::Activate()
{
    ...
    if (!this->__Store__Subsystem->IsHandleValid(Handle))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem value not valid when Activate() was called"));
        this->OnCallFailed
            .Broadcast(false, FUniqueNetIdRepl(), FExternalIdQueryOptionsBP(), TArray<FString>(), TEXT(""));
        return;
    }
    if (!(this->__Store__UserId.IsValid() && this->__Store__UserId.GetUniqueNetId().IsValid()))
    {
    ...
    if (!(ValidateArrayElements<FString>(this->__Store__ExternalIds, [](const FString &Val) {
            return true;
        })))
    {
    ...
    auto __DelegateHandle_OnQueryExternalIdMappingsComplete =
        IOnlineUser::FOnQueryExternalIdMappingsComplete::CreateUObject(
            this,
            &UOnlineUserSubsystemQueryExternalIdMappings::HandleCallback_OnQueryExternalIdMappingsComplete);

    if (!Handle->QueryExternalIdMappings(
            this->__Store__UserId.GetUniqueNetId().ToSharedRef().Get(),
            this->__Store__QueryOptions.ToNative(),
            ConvertArrayElements<FString, FString>(
                this->__Store__ExternalIds,
                [](const FString &Val) {
                    return Val;
                }),
            __DelegateHandle_OnQueryExternalIdMappingsComplete))
    {
...
void UOnlineUserSubsystemQueryExternalIdMappings::HandleCallback_OnQueryExternalIdMappingsComplete(
    bool bWasSuccessful,
    const FUniqueNetId &UserId,
    ...
    const FString &Error)
{
    ...
    this->OnQueryExternalIdMappingsComplete.Broadcast(
        bWasSuccessful,
        ConvertDangerousUniqueNetIdToRepl(UserId),
        FExternalIdQueryOptionsBP::FromNative(QueryOptions),
        ConvertArrayElements<FString, FString>(
            ExternalIds,
            [](const FString &Val) {
                return Val;
...
void UOnlineUserSubsystem::GetExternalIdMappings(
    const FExternalIdQueryOptionsBP &QueryOptions,
    const TArray<FString> &ExternalIds,
    TArray<FUniqueNetIdRepl> &OutIds)
{
    ...
    if (!(ValidateArrayElements<FString>(ExternalIds, [](const FString &Val) {
            return true;
        })))
    {
        return;
    ...
    Handle->GetExternalIdMappings(
        QueryOptions.ToNative(),
        ConvertArrayElements<FString, FString>(
            ExternalIds,
            [](const FString &Val) {
                return Val;
        ...
        __StoreTemp__OutIds);
    OutIds =
        ConvertArrayElements<FUniqueNetIdPtr, FUniqueNetIdRepl>(__StoreTemp__OutIds, [](const FUniqueNetIdPtr &Val) {
            return FUniqueNetIdRepl(Val);
        });
}

FUniqueNetIdRepl UOnlineUserSubsystem::GetExternalIdMapping(
    const FExternalIdQueryOptionsBP &QueryOptions,
