...
FExternalIdQueryOptions FExternalIdQueryOptionsBP::ToNative() const
{
...
FString UUserOnlineAccountRef::GetAuthAttribute(const FString &Key, bool &Found)
{
    if (!this->UserOnlineAccount.IsValid())
    {
        Found = false;
...
bool UUserOnlineAccountRef::SetUserAttribute(const FString &Key, const FString &Value)
{
    if (!this->UserOnlineAccount.IsValid())
    {
        return false;
