...
UOnlineSessionInfo *UOnlineSessionInfo::FromNative(const TSharedPtr<class FOnlineSessionInfo> &InSessionInfo)
{
...
FOnlineSessionSettingsBP FOnlineSessionSettingsBP::FromNative(const FOnlineSessionSettings &InObj)
{
    ...
    Result.bUseLobbiesIfAvailable = InObj.bUseLobbiesIfAvailable;
    Result.bUseLobbiesVoiceChatIfAvailable = InObj.bUseLobbiesVoiceChatIfAvailable;
    ...
    for (auto MV : InObj.MemberSettings)
    {
        TMap<FName, FOnlineSessionSettingBP> PerMemberSettings;
        for (auto KV : MV.Value)
        {
            PerMemberSettings.Add(KV.Key, FOnlineSessionSettingBP::FromNative(KV.Value));
        }
        FOnlineSessionMemberSettingsBP PerMember;
        PerMember.MemberUserId = MV.Key;
        PerMember.MemberSettings = PerMemberSettings;
        Result.MemberSettings.Add(PerMember);
    }
    return Result;
}

FOnlineSessionSettings FOnlineSessionSettingsBP::ToNative() const
{
    ...
    Result.bUseLobbiesIfAvailable = this->bUseLobbiesIfAvailable;
    Result.bUseLobbiesVoiceChatIfAvailable = this->bUseLobbiesVoiceChatIfAvailable;
    Result.BuildUniqueId = this->BuildUniqueId;
    for (auto KV : this->Settings)
    {
        Result.Settings.Add(KV.Key, KV.Value.ToNative());
    }
    for (auto MS : this->MemberSettings)
    {
        auto UserId = MS.MemberUserId.GetUniqueNetId();
        if (UserId.IsValid())
        {
            FSessionSettings PerMemberSettings;
            for (auto KV : MS.MemberSettings)
            {
                PerMemberSettings.Add(KV.Key, KV.Value.ToNative());
            }
            Result.MemberSettings.Add(UserId.ToSharedRef(), PerMemberSettings);
        }
    }
    return Result;
}
...
FOnlineSession FOnlineSessionBP::ToNative() const
{
...
FOnlineSessionSearchResult FOnlineSessionSearchResultBP::ToNative() const
{
