...
UParty *UParty::FromNative(const FOnlinePartyConstPtr &InObj)
{
...
UPartyId *UParty::GetPartyId() const
{
...
FUniqueNetIdRepl UParty::GetLeaderId() const
{
    if (auto PinnedParty = this->Party.Pin())
    {
        if (PinnedParty.IsValid())
...
FOnlinePartyConfiguration UParty::GetPartyConfiguration() const
{
    if (auto PinnedParty = this->Party.Pin())
    {
        if (PinnedParty.IsValid())
        {
            if (PinnedParty->LeaderId.IsValid())
            {
                return FOnlinePartyConfiguration::FromNative(*PinnedParty->GetConfiguration());
            }
        }
    }

    return FOnlinePartyConfiguration();
}
...
UBlueprintPartyMember *UBlueprintPartyMember::FromNative(const FOnlinePartyMemberConstPtr &InObj)
{
...
UReadablePartyData *UReadablePartyData::FromNative(const TSharedPtr<const FOnlinePartyData> &InObj)
{
    if (!InObj.IsValid())
    {
        return nullptr;
...
UMutablePartyData *UMutablePartyData::FromNative(const TSharedRef<FOnlinePartyData> &InObj)
{
    auto Ref = NewObject<UMutablePartyData>();
    Ref->AssignFromConstPartyData(*InObj);
...
UMutablePartyData *UMutablePartyData::FromNative(const TSharedPtr<FOnlinePartyData> &InObj)
{
    if (!InObj.IsValid())
    {
        return nullptr;
...
void UMutablePartyData::SetAttribute(const FString &AttrName, const FVariantDataBP &AttrValue)
{
