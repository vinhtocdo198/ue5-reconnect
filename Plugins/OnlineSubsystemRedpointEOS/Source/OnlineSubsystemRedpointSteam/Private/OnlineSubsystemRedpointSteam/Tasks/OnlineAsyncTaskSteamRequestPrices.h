// Copyright June Rhodes. All Rights Reserved.

#pragma once

#include "RedpointEOSBuild/BuildEnvironment.h"

#include "OnlineAsyncTaskManager.h"
#include "OnlineSubsystemRedpointSteam/SDK.h"
#include "OnlineSubsystemRedpointSteam/Tasks/OnlineAsyncTaskSteamDelegates.h"

#if defined(REDPOINT_EOS_STEAM_ENABLED)

namespace REDPOINT_EOS_FILE_NS_ID(3665562259, Redpoint::EOS::OnlineSubsystemRedpointSteam::Tasks)
{
using namespace ::Redpoint::EOS::OnlineSubsystemRedpointSteam::Tasks;
using namespace ::Redpoint::EOS::OnlineSubsystemRedpointSteam::Inventory;

class FOnlineAsyncTaskSteamRequestPrices : public FOnlineAsyncTaskBasic<class FOnlineSubsystemSteam>
{
private:
    bool bInit;
    SteamAPICall_t PriceCallbackHandle;
    SteamInventoryRequestPricesResult_t PriceCallbackResults;
    FSteamRequestPricesComplete Delegate;
    FString FailureContext;
    FString ResultCurrencyCode;
    TArray<FSteamItemDefinitionPriceRaw> ResultPrices;

public:
    FOnlineAsyncTaskSteamRequestPrices(FSteamRequestPricesComplete InDelegate);
    UE_NONCOPYABLE(FOnlineAsyncTaskSteamRequestPrices);
    virtual ~FOnlineAsyncTaskSteamRequestPrices() = default;

    virtual FString ToString() const override
    {
        return TEXT("FOnlineAsyncTaskSteamRequestPrices");
    }

    virtual void Tick() override;
    virtual void Finalize() override;
};

}

namespace Redpoint::EOS::OnlineSubsystemRedpointSteam::Tasks
{
REDPOINT_EOS_FILE_NS_EXPORT(
    3665562259,
    Redpoint::EOS::OnlineSubsystemRedpointSteam::Tasks,
    FOnlineAsyncTaskSteamRequestPrices)
}

#endif