...
// This is a copy of IEOSSDKManager.h so we don't have to depend on EOSShared.
...
DECLARE_MULTICAST_DELEGATE_TwoParams(
    FEOSSDKManagerOnDefaultPlatformConfigNameChanged,
    const FString &NewName,
    const FString &OldName);
DECLARE_MULTICAST_DELEGATE_TwoParams(
    FEOSSDKManagerOnPreCreateNamedPlatform,
    const FEOSSDKPlatformConfig &Config,
...
public:
    ...
    virtual const FEOSSDKPlatformConfig *GetPlatformConfig(
        const FString &PlatformConfigName,
        bool bLoadIfMissing = false) = 0;
    virtual bool AddPlatformConfig(
        const FEOSSDKPlatformConfig &PlatformConfig,
        bool bOverwriteExistingConfig = false) = 0;
    ...
    /**
     * Create a platform handle for a platform config name. Config is loaded from .ini files if it was not added with
     * AddPlatformConfig. If a platform handle already exists for the config name, this will return a shared pointer to
     * that handle and not create a new one.
     */
    ...
    virtual void LogAuthInfo(const EOS_HPlatform Platform, const EOS_EpicAccountId LoggedInAccount, int32 Indent = 0)
        const = 0;
    virtual void LogUserInfo(
    ...
    virtual void LogPresenceInfo(
        ...
        const EOS_EpicAccountId TargetAccount,
        int32 Indent = 0) const = 0;
    virtual void LogFriendsInfo(const EOS_HPlatform Platform, const EOS_EpicAccountId LoggedInAccount, int32 Indent = 0)
        const = 0;
    virtual void LogConnectInfo(const EOS_HPlatform Platform, const EOS_ProductUserId LoggedInAccount, int32 Indent = 0)
        const = 0;

    /** Assign ownership of a callback object, to be free'd after EOS_Shutdown */
    virtual void AddCallbackObject(TUniquePtr<FCallbackBase> CallbackObj) = 0;
