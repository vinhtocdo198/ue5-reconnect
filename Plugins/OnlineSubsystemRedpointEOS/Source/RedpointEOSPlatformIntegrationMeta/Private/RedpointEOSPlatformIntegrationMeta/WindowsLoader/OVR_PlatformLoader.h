...
static bool OVR_GetOculusBaseDirectory(FilePathCharType *directoryPath, size_t directoryPathCapacity)
{
    DWORD length = directoryPathCapacity;
    HKEY key = 0;
    // Query the 32bit key even if 64bit build
    LONG result =
        RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"Software\\Oculus VR, LLC\\Oculus", 0, KEY_READ | KEY_WOW64_32KEY, &key);
    if (result != ERROR_SUCCESS)
    {
        return false;
    }

    result = RegGetValueW(key, NULL, L"Base", RRF_RT_REG_SZ | RRF_ZEROONFAILURE, NULL, (LPBYTE)directoryPath, &length);
    RegCloseKey(key);
...
#define OVR_FILE_PATH_SEPARATOR L"\\"
    ...
    if (OVR_GetOculusBaseDirectory(oculusInstallDir, _countof(oculusInstallDir)))
    {
        ...
        {
            baseDirLength = OVR_wstrlcat(oculusInstallDir, OVR_FILE_PATH_SEPARATOR, _countof(oculusInstallDir));
        }

#define OVR_RUNTIME_PATH L"Support\\oculus-runtime\\"

        if ((baseDirLength > 0) && (baseDirLength + _countof(OVR_RUNTIME_PATH) < _countof(oculusInstallDir)))
        {
    ...
    *outResult =
        lastOpenResult != ovrPlatformInitialize_Uninitialized ? lastOpenResult : ovrPlatformInitialize_UnableToVerify;
    return moduleHandle;
}
...
OVRPL_DECLARE_IMPORT(
    ...
    (ovrID appId, const char *accessToken, ovrKeyValuePair *configOptions, size_t numConfigOptions));
