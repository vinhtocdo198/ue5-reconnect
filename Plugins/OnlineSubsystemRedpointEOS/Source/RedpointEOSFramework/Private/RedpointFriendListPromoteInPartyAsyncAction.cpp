...
#include "RedpointFriendListPromoteInPartyAsyncAction.h"
...
URedpointFriendListPromoteInPartyAsyncAction *URedpointFriendListPromoteInPartyAsyncAction::PromoteInParty(
    URedpointFriendListEntry *FriendListEntry)
{
    URedpointFriendListPromoteInPartyAsyncAction *Node = NewObject<URedpointFriendListPromoteInPartyAsyncAction>();
    Node->Entry = FriendListEntry;
    return Node;
}

void URedpointFriendListPromoteInPartyAsyncAction::Activate()
{
    ...
    if (!PartySystem->PromoteMember(
            *GetUniqueNetId(Entry->BoundForLocalUserId),
            *PrimaryParty->PartyId,
            *Entry->UserId,
            FOnPromotePartyMemberComplete::CreateWeakLambda(
                this,
                [this](
                    const FUniqueNetId &,
                    const FOnlinePartyId &,
                    const FUniqueNetId &,
                    const EPromoteMemberCompletionResult) {
