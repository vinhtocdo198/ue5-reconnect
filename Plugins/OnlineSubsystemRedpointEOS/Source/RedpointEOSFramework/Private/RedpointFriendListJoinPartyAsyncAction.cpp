...
#include "RedpointFriendListJoinPartyAsyncAction.h"
...
URedpointFriendListJoinPartyAsyncAction *URedpointFriendListJoinPartyAsyncAction::JoinParty(
    URedpointFriendListEntry *FriendListEntry)
{
    URedpointFriendListJoinPartyAsyncAction *Node = NewObject<URedpointFriendListJoinPartyAsyncAction>();
    Node->Entry = FriendListEntry;
    return Node;
}

void URedpointFriendListJoinPartyAsyncAction::Activate()
{
    ...
    auto AdvertisedParty = PartySystem->GetAdvertisedParty(
        *GetUniqueNetId(Entry->BoundForLocalUserId),
        *Entry->UserId,
        PartySystem->GetPrimaryPartyTypeId());
    if (!AdvertisedParty.IsValid())
    {
        this->OnComplete.Broadcast();
        return;
    }
    ...
    if (!PartySystem->JoinParty(
            *GetUniqueNetId(Entry->BoundForLocalUserId),
            *AdvertisedParty,
            FOnJoinPartyComplete::CreateWeakLambda(
                this,
                [this](const FUniqueNetId &, const FOnlinePartyId &, const EJoinPartyCompletionResult, const int32) {
