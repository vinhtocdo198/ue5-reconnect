...
#define REDPOINT_EOSSDK_API_CALL_SYNC_GET_IMPL(CallName)                                                               \
    REDPOINT_EOS_DEFINE_API_CALL_LLM_TAG(CallName);                                                     \
                                                                                                                       \
    EOS_EResult F##CallName::Execute(const FPlatformHandle &InHandle, const Options &InOptions, Result &OutResult)     \
    {                                                                                                                  \
        checkf(IsInGameThread(), TEXT("Calls to the EOS SDK can only be made on the game thread!"));                   \
                                                                                                                       \
        return Execute(InHandle->Get<NativeHandle>(), InOptions, OutResult);                                           \
    }                                                                                                                  \
    EOS_EResult F##CallName::Execute(NativeHandle InHandle, const Options &InOptions, Result &OutResult)               \
    {                                                                                                                  \
        checkf(IsInGameThread(), TEXT("Calls to the EOS SDK can only be made on the game thread!"));                   \
                                                                                                                       \
        ...
        NativeResult _NativeResult = {};                                                                               \
                                                                                                                       \
        EOS_EResult ResultCode = NativeFunction(InHandle, &_NativeOptions, &_NativeResult);                            \
                                                                                                                       \
        OutResult = {};                                                                                                \
        if (ResultCode == EOS_EResult::EOS_Success)                                                                    \
        {                                                                                                              \
            MapResult(OutResult, _NativeResult, *MakeShared<NativeConverter>());                                       \
        }                                                                                                              \
                                                                                                                       \
        UE_LOG(                                                                                                        \
            LogRedpointEOSAPI,                                                                                         \
