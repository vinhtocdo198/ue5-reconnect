...
#define REDPOINT_EOSSDK_API_CALL_SYNC_GET_STRING_SIGNED_COUNT_IMPL(CallName)                                           \
    ...
    {                                                                                                                  \
        ...
        MapOptions(_NativeOptions, InOptions, *Allocator);                                                             \
                                                                                                                       \
        int32_t NativeResultStackBufferLength = 1;                                                                     \
        char *NativeResultStackBuffer =                                                                                \
            (char *)FMemory::MallocZeroed((static_cast<uint32_t>(NativeResultStackBufferLength) + 1) * sizeof(char));  \
        OutResultCode = NativeFunction(                                                                                \
            InHandle->Get<NativeHandle>(),                                                                             \
            &_NativeOptions,                                                                                           \
            NativeResultStackBuffer,                                                                                   \
            &NativeResultStackBufferLength);                                                                           \
        if (OutResultCode == EOS_EResult::EOS_LimitExceeded || NativeResultStackBufferLength < 0)                      \
        {                                                                                                              \
            FMemory::Free(NativeResultStackBuffer);                                                                    \
            NativeResultStackBuffer = (char *)FMemory::MallocZeroed(                                                   \
                (static_cast<uint32_t>(NativeResultStackBufferLength) + 1) * sizeof(char));                            \
            OutResultCode = NativeFunction(                                                                            \
                InHandle->Get<NativeHandle>(),                                                                         \
                &_NativeOptions,                                                                                       \
                NativeResultStackBuffer,                                                                               \
