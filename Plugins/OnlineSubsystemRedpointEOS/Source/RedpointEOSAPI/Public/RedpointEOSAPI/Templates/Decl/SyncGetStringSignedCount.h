...
#include "RedpointEOSAPI/Templates/NativeCall/SyncGetStringSignedCountInfo.h"
...
#define REDPOINT_EOSSDK_API_CALL_SYNC_GET_STRING_SIGNED_COUNT_BEGIN(CallNamespace, CallName, CallVersion)              \
private:                                                                                                               \
    REDPOINT_EOS_DECLARE_API_CALL_LLM_TAG(CallNamespace, CallName);                                                    \
    typedef EOS_H##CallNamespace NativeHandle;                                                                         \
    typedef ::Redpoint::EOS::API::Templates::NativeCall::TSyncGetStringSignedCountInfo<                                \
    ...
    NativeFunction(NativeHandle Handle, const NativeOptions *Options, char *OutBuffer, int32_t *InOutBufferLength)     \
    {                                                                                                                  \
...
#define REDPOINT_EOSSDK_API_CALL_SYNC_GET_STRING_SIGNED_COUNT_END()                                                    \
public:                                                                                                                \
    static void Execute(                                                                                               \
