...
namespace REDPOINT_EOS_FILE_NS_ID(282591651, Redpoint::EOS::Tests::RoomSystem::Presence)
{
...
static TTask<void>::With<ETaskBinding::Static> TestByOverlayAccept(
    ...
    TArray<FPlatformHandle> Platforms)
{
    ...
    auto RoomJoinRequestedTask = TTask<FRoomRequestedJoinRef>::Deferred();
    FDelegateHandle RoomJoinRequestedDelegateHandle;
    {
        ...
        RoomSystem->OnRoomInviteReceived(NAME_Game).Remove(RoomInviteDelegateHandle);

        RoomJoinRequestedDelegateHandle =
            RoomSystem->OnRoomJoinRequested(NAME_Game).AddLambda([RoomJoinRequestedTask](
                                                                     const FRoomEventContext &EventContext,
                                                                     const FRoomRequestedJoinRef &RoomRequestedJoin) {
                RoomJoinRequestedTask.SetValue(RoomRequestedJoin);
            });
    }
    ...
    // Simulate an overlay "accept" event using internal API access on the room system.
    {
        ...
        RoomSystem->OnRoomInviteLocallyAcceptedJoinRequiredFromSDKEvent(
            ReceivedRoomInvite->GetRoomInviteId(),
            ReceivedRoomInvite->GetRoomInviteRecipientUserId());
    }
    ...
    {
        auto JoinRoomRequest = co_await RoomJoinRequestedTask;
        ...
        auto JoinRequest = FJoinRoomRequest(ERoomScope::User, JoinRoomRequest);
...
static auto JoinPresenceRoomByOverlayAccept = MakeShared<FAsyncAutomationTest>(
    TEXT("Redpoint.EOS.RoomSystem.Presence.JoinPresenceRoom.ByOverlayAccept"),
    TestWithPlatforms(TArray<int32>{1, 1}, TestByOverlayAccept));
