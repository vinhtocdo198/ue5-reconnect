...
void FLogUserIntoLocalPlatformNode::Execute(
    ...
    FAuthenticationGraphNodeOnDone InOnDone)
{
    ...
    // Ask the runtime platform integration service to ensure the user is signed in on the local platform.
    auto CallContext = MakeShared<FRuntimePlatformServiceCallContext>(
        InState->PlatformHandle,
        InState->LocalUserNum,
        InState->GetWorld());
    AuthService->EnsureSignedInOnLocalPlatform(
        CallContext,
